...
function DeleteItem(keyword) {location.href='%s?domain=%s&session=%s&keyword=' + keyword;}\n\
function ModifyItem(keyword, memo) {\
opeform.keyword.value=keyword; opeform.memo.value=memo; opeform.memo.focus();}\n\
...
name=keyword /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text \
value=\"\" tabindex=2 name=memo /></TD></TR><TR><TD></TD><TD></TD><TD>\n\
<INPUT type=submit tabindex=3 value=\"%s\" onclick=\"\n\
if (0 == opeform.keyword.value.length) {return false;} return true;\" \n\
/></TD></TR></TABLE></FORM>\n\
...
static void	list_ui_modify_list(const char *domain, const char *keyword,
	const char *memo);

static void list_ui_remove_item(const char *domain, const char *keyword);
			
static void list_ui_error_html(const char *error_string);

static void list_ui_main_html(const char *domain, const char *session);

static void list_ui_broadcast_list(const char *domain);

static BOOL list_ui_get_self(char *url_buff, int length);

static void list_ui_unencode(char *src, char *last, char *dest);

static void list_ui_approve_html(const char *domain, const char *session,
	const char *action);
	...
	if (0 == strcmp(request, "POST")) {
		...
		if (NULL == query) {
			...
			if (NULL == ptr2) {
                ...
                list_ui_approve_html(domain, session, action);
                return 0;
			...
			if (NULL == ptr2) {
				...
				list_ui_remove_item(domain, temp_keyword);
				list_ui_main_html(domain, session);
				return 0;
			}
			...
			list_ui_modify_list(domain, temp_keyword, memo);
			list_ui_main_html(domain, session);
			return 0;
		}
	} else {
...
static void list_ui_main_html(const char *domain, const char *session)
{
	...
	snprintf(list_path, 256, "%s/keyword.txt", g_domain_path);
	...
	if (NULL == pfile) {
		system_log_info("[list_ui]: fail to open list file %s", list_path);
		list_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_INTERNAL",
			language));
		return;
	}
	...
	for (i=0; i<item_num; i++) {
		list_ui_encode_squote(pitem[i].keyword, temp_name);
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, pitem[i].keyword,
	...
	const char *memo)
{
	...
	item_num = list_file_get_item_num(pfile);
	for (i=0; i<item_num; i++) {
		if (0 == strcasecmp(keyword, pitem[i].keyword)) {
			break;
		}
	}
	...
	if (i < item_num) {
		...
		for (j=0; j<item_num; j++) {
			...
			list_ui_encode_line(pitem[j].keyword, temp_keyword);
			list_ui_encode_line(pitem[j].memo, temp_memo);
			len = sprintf(temp_line, "%s\t%s\n", temp_keyword, temp_memo);
			write(fd, temp_line, len);

		}
		list_ui_encode_line(keyword, temp_keyword);
...
static void list_ui_broadcast_list(const char *domain)
{
	...
	if (NULL == pconfig) {
		...
		if (NULL == str_value) {
			...
			keyword_type = atoi(str_value);
			if (keyword_type < 0 || keyword_type > 1) {
				keyword_type = 0;
			}
		}
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		for (i=0; i<item_num; i++) {
			list_ui_encode_line(pitem[i].keyword, temp_keyword);
	...
	if (FALSE == data_source_get_homedir(domain, g_domain_path) ||
		...
		if (NULL == pconfig) {
			...
			if (NULL == str_value) {
				...
				strncpy(language, str_value, 32);
			}
			config_file_free(pconfig);
		}
	}
	if (0 == strcmp(action, "allow")) {
		snprintf(buff, 1024, "message_insulation.hook activate %s", session);
		...
		while ((direntp = readdir(dirp)) != NULL) {
			...
			sprintf(temp_path, "%s/%s/queue/insulation/%s",
				g_mount_path, direntp->d_name, session);
