...
#define HTML_SEARCH_7	" name=group><INPUT type=hidden value="
...
static void backup_ui_search_html(const char *groupname, const char *session);

static void backup_ui_result_html(const char *groupname, const char *session,
	const char *username);
...
int backup_ui_run()
{
	...
	if (0 == strcmp(request, "POST")) {
		...
		if (NULL == ptoken || 0 != strcasecmp(ptoken + 1, pdomain)) {
			backup_ui_result_html(groupname, session, lang_resource_get(g_lang_resource,
		...
		if (FALSE == data_source_check_group(groupname, username, &bresult) ||
			FALSE == bresult) {
			backup_ui_result_html(groupname, session, lang_resource_get(g_lang_resource,
				"MSGERR_USERNAME_GROUP", language));
			return 0;
		}
		...
		if (NULL == query) {
			...
			backup_ui_search_html(groupname, session);
			return 0;
		}
	} else {
...
static void backup_ui_search_html(const char *groupname, const char *session)
{
	...
	printf(HTML_SEARCH_12, lang_resource_get(g_lang_resource,"MSGERR_USERNAME_EMPTY", language),
			lang_resource_get(g_lang_resource,"MSGERR_USERNAME_ERROR", language), pdomain,
	...
	const char *msg)
{
	...
	printf(url_buff);
	printf("?group=%s&session=%s", groupname, session);
