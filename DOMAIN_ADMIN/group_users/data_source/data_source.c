...
BOOL data_source_query(const char *groupname, const char *username,
	const char *title, const char *real_name, const char *nickname,
	const char *tel, const char *cell, const char *homeaddress,
	const char *memo, int size_min, int size_max, time_t create_min,
...
RETRYING:
	...
	sprintf(sql_string, "SELECT username, real_name, max_size, "
		"create_day, privilege_bits, address_status, address_type,"
		" maildir, memo, nickname, tel, cell, homeaddress, title, "
	...
	for (j=0; j<rows; j++) {
		myrow = mysql_fetch_row(pmyres);
		if (ADDRESS_TYPE_NORMAL != atoi(myrow[6]) &&
			ADDRESS_TYPE_ALIAS != atoi(myrow[6])) {
			continue;
		}
		if (NULL != username) {
		...
		if (NULL != title && NULL == strcasestr(myrow[13], title)) {
			continue;
		}
		if (NULL != real_name && NULL == strcasestr(myrow[1], real_name)) {
			continue;
		}
		if (NULL != memo && NULL == strcasestr(myrow[8], memo)) {
			continue;
		}
		if (NULL != nickname && NULL == strcasestr(myrow[9], nickname)) {
			continue;
		}
		if (NULL != tel && NULL == strcasestr(myrow[10], tel)) {
			continue;
		}
		if (NULL != cell && NULL == strcasestr(myrow[11], cell)) {
			continue;
		}
		if (NULL != homeaddress && NULL == strcasestr(myrow[12], homeaddress)) {
			continue;
		}
		if (size_min > 0 && size_min > atoi(myrow[2])) {
			continue;
		}
		if (size_max > 0 && size_max < atoi(myrow[2])) {
			continue;
		}
		if (create_min > 0) {
			memset(&tmp_tm, 0, sizeof(tmp_tm));
			strptime(myrow[3], "%Y-%m-%d", &tmp_tm);
		...
		if (address_status >= 0 && address_status != (atoi(myrow[5])&0x3)) {
			continue;
		}

		if (address_type >= 0 && address_type != atoi(myrow[6])) {
			continue;
		}
		
		if (sub_type >= 0 && sub_type != atoi(myrow[14])) {
			continue;
		}
		...
		strcpy(pdata->item.title, myrow[13]);
		strcpy(pdata->item.real_name, myrow[1]);
		strcpy(pdata->item.memo, myrow[8]);
		strcpy(pdata->item.nickname, myrow[9]);
		strcpy(pdata->item.tel, myrow[10]);
		strcpy(pdata->item.cell, myrow[11]);
		strcpy(pdata->item.homeaddress, myrow[12]);
		strcpy(pdata->item.maildir, myrow[7]);
		pdata->item.max_size = atoi(myrow[2]);
		memset(&tmp_tm, 0, sizeof(tmp_tm));
		strptime(myrow[3], "%Y-%m-%d", &tmp_tm);
		pdata->item.create_day = mktime(&tmp_tm);
		pdata->item.privilege_bits = (atoi(myrow[4])&0xFF);
		pdata->item.address_status = (atoi(myrow[5])&0x3);
		pdata->item.address_type = atoi(myrow[6]);
		pdata->item.sub_type = atoi(myrow[14]);
...
BOOL data_source_add_user(const char *groupname, const char *username,
	const char *password, const char *title, const char *real_name,
	const char *nickname, const char *tel, const char *cell,
	const char *homeaddress, const char *memo, const char *maildir,
	int max_size, int max_file, int privilege_bits, int address_status,
	int sub_type, int *presult, int *puser_id)
{
	LOCKD lockd;
	time_t now_time;
	int i, j, rows;
...
RETRYING:
	...
	if (NULL == (pmysql = mysql_init(NULL)) ||
		...
		if (NULL != pmysql) {
			system_log_info("[data_source]: fail to connect to "
	...
	sprintf(sql_string, "SELECT id, max_size, max_user, privilege_bits,"
			" group_status FROM groups WHERE groupname='%s'", temp_group);
	...
	if (total_users >= group_user) {
		mysql_close(pmysql);
		*presult = ADD_RESULT_GROUPUSERFULL;
		locker_client_unlock(lockd);
		return TRUE;
	}

	if (total_size + max_size > group_size) {
		mysql_close(pmysql);
		*presult = ADD_RESULT_GROUPSIZEFULL;
		locker_client_unlock(lockd);
		return TRUE;
	}
	...
	if (total_users >= domain_user) {
		mysql_close(pmysql);
		*presult = ADD_RESULT_DOMAINUSERFULL;
		locker_client_unlock(lockd);
		return TRUE;
	}
	...
	if (total_size + max_size > domain_size) {
		mysql_close(pmysql);
		*presult = ADD_RESULT_DOMAINSIZEFULL;
		locker_client_unlock(lockd);
		return TRUE;
	}
	...
	snprintf(sql_string, 4096, "INSERT INTO users (username, password, "
		...
		temp_user, password, temp_title, temp_real, temp_nick, temp_tel,
		temp_cell, temp_home, temp_memo, domain_id, group_id, maildir,
	...
	*puser_id = mysql_insert_id(pmysql);
	sprintf(sql_string, "SELECT aliasname FROM "
		"aliases WHERE mainname='%s'", temp_domain);
	...
	for (j=0; j<rows; j++) {
		...
		snprintf(sql_string, 4096, "INSERT INTO users (username, password, "
			...
			"'%s', %d, %d, 3)", temp_address, password, temp_title, temp_real,
			temp_nick, temp_tel, temp_cell, temp_home, temp_memo, domain_id,
			group_id, maildir, max_size, max_file, str_create, temp_privilege,
			temp_status);
		if (0 != mysql_query(pmysql, sql_string)) {
			system_log_info("[data_source]: fail to query mysql"
					" server, reason: %s", mysql_error(pmysql));
			system_log_info("[data_source]: fail to insert virtual"
					" address %s into database", virtual_address);
		}
	}
	mysql_free_result(pmyres);
	mysql_close(pmysql);
...
BOOL data_source_add_alias(const char *groupname, const char *username,
	const char *alias, int *presult)
{
	int i, j, rows;
	int domain_id;
	int group_id;
...
RETRYING:
	...
	sprintf(sql_string, "SELECT password, real_name, domain_id, group_id, "
		...
		"privilege_bits, address_status, address_type, memo, nickname, tel, "
		"cell, homeaddress, title FROM users WHERE username='%s'", temp_user);
	...
	if (group_id != atoi(myrow[3])) {
		mysql_free_result(pmyres);
		mysql_close(pmysql);
		*presult = ALIAS_RESULT_GROUPERR;
		locker_client_unlock(lockd);
		return TRUE;
	}
...
BOOL data_source_edit_user(const char *groupname, const char *username,
	const char *password, const char *title, const char *real_name,
	const char *nickname, const char *tel, const char *cell,
	const char *homeaddress, const char *memo, int max_size,
	int privilege_bits, int address_status, int *presult)
{
	int i, j, k;
	int rows, rows1;
	int group_id;
...
RETRYING:
	...
	data_source_encode_squote(groupname, temp_group);
	sprintf(sql_string, "SELECT id, max_size FROM groups WHERE groupname='%s'",
		temp_group);
	if (0 != mysql_query(pmysql, sql_string) ||
		NULL == (pmyres = mysql_store_result(pmysql))) {
		system_log_info("[data_source]: fail to query mysql server, "
	...
	data_source_encode_squote(username, temp_user);
	sprintf(sql_string, "SELECT id, group_id, domain_id, max_size, "
		"privilege_bits, address_status, address_type FROM users WHERE "
		"username='%s'", temp_user);
	...
	if (max_size != atoi(myrow[3])) {
		...
		if (max_size + total_size - atoi(myrow[3]) > group_size) {
			mysql_free_result(pmyres);
			mysql_close(pmysql);
			*presult = EDIT_RESULT_GROUPSIZEFULL;
			locker_client_unlock(lockd);
			return TRUE;
		}
		...
		mysql_free_result(pmyres1);
		if (max_size + total_size - atoi(myrow[3]) > domain_size) {
			mysql_free_result(pmyres);
			mysql_close(pmysql);
			*presult = EDIT_RESULT_DOMAINSIZEFULL;
			locker_client_unlock(lockd);
			return TRUE;
		}
	...
	original_privilege = atoi(myrow[4]);
	original_status = atoi(myrow[5]);
	original_privilege &= 0xFFFF00;
	original_status &= 0x3C;
	privilege_bits |= original_privilege;
	address_status |= original_status;
	address_type = atoi(myrow[6]);
	mysql_free_result(pmyres);
	...
	for (j=0; j<rows; j++) {
		...
		if ('\0' == password[0]) {
			...
			snprintf(sql_string, 4096, "UPDATE users SET password='%s', "
				...
				"username='%s'", password, temp_title, temp_real, temp_nick,
				temp_tel, temp_cell, temp_home, temp_memo, group_id, max_size,
				privilege_bits, address_status, temp_alias);
		}
		if (0 != mysql_query(pmysql, sql_string)) {
			system_log_info("[data_source]: fail to query mysql server, "
				"reason: %s", mysql_error(pmysql));
		...
		for (k=0; k<rows1; k++) {
			...
			if ('\0' == password[0]) {
				...
				snprintf(sql_string, 4096, "UPDATE users SET password='%s', "
					...
					"cell='%s', homeaddress='%s', memo='%s', group_id=%d, "
					"max_size=%d, privilege_bits=%d, address_status=%d WHERE "
					"username='%s'", password, temp_title, temp_real,
					temp_nick, temp_tel, temp_cell, temp_home, temp_memo,
					group_id, max_size, privilege_bits, address_status,
					temp_address);
			}
			if (0 != mysql_query(pmysql, sql_string)) {
				system_log_info("[data_source]: fail to query mysql server, "
...
BOOL data_source_remove_user(const char *groupname, const char *username,
	BOOL *pb_alias, DATA_COLLECT *pcollect)
{
	int i, j, k;
	int rows, rows1;
	int temp_id;
...
RETRYING:
	...
	data_source_encode_squote(groupname, temp_group);
	sprintf(sql_string, "SELECT id FROM groups"
			" WHERE groupname='%s'", temp_group);
	...
	data_source_encode_squote(username, temp_user);
	snprintf(sql_string, 4096, "SELECT id, group_id, address_type"
					" FROM users WHERE username='%s'", temp_user);
	...
	if (ADDRESS_TYPE_ALIAS == address_type) {
		...
		for (j=0; j<rows; j++) {
			...
			strcpy(pat, myrow[0]);
			data_source_encode_squote(virtual_address, temp_address);
			sprintf(sql_string, "DELETE FROM users "
				"WHERE username='%s'", temp_address);
			if (0 != mysql_query(pmysql, sql_string)) {
				system_log_info("[data_source]: fail to query mysql"
						" server, reason: %s", mysql_error(pmysql));
				system_log_info("[data_source]: fail to delete virtual"
						" address %s from database", virtual_address);
			}
		}

		mysql_free_result(pmyres);

		sprintf(sql_string, "DELETE FROM aliases"
			" WHERE aliasname='%s'", temp_user);
		if (0 != mysql_query(pmysql, sql_string)) {
			system_log_info("[data_source]: fail to query mysql"
					" server, reason: %s", mysql_error(pmysql));
			mysql_close(pmysql);
		...
		sprintf(sql_string, "DELETE FROM associations"
	...
	sprintf(sql_string, "SELECT aliasname FROM "
		"aliases WHERE mainname='%s'", temp_user);
	...
	for (j=0; j<rows; j++) {
		...
		data_source_encode_squote(myrow[0], temp_alias);
		sprintf(sql_string, "DELETE FROM users "
			"WHERE username='%s'", temp_alias);
		if (0 != mysql_query(pmysql, sql_string)) {
			system_log_info("[data_source]: fail to query mysql"
					" server, reason: %s", mysql_error(pmysql));
			system_log_info("[data_source]: fail to delete "
				"alias address %s in database", myrow[0]);
		}
		mysql_data_seek(pmyres1, 0);
		for (k=0; k<rows1; k++) {
			myrow1 = mysql_fetch_row(pmyres1);
			...
			if (0 != mysql_query(pmysql, sql_string)) {
				system_log_info("[data_source]: fail to query mysql"
						" server, reason: %s", mysql_error(pmysql));
				system_log_info("[data_source]: fail to delete alias"
						" address %s in database", virtual_address);
			}
		}
		if (NULL != pcollect) {
			pdata = (DATA_NODE*)malloc(sizeof(DATA_NODE));
...
BOOL data_source_num_user(const char *groupname, int *pnum)
{
	int i, j, rows;
	int group_id;
	char temp_name[256];
...
RETRYING:
	...
	sprintf(sql_string, "SELECT privilege_bits FROM groups "
		"WHERE groupname='%s'", temp_name);
