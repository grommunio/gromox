...
RETRYING:
	...
	*pprivilege_bits = 0;
	...
	sprintf(sql_string, "SELECT id, title, max_size, max_user, create_day, "
		"privilege_bits FROM groups WHERE groupname='%s'", temp_name);
	...
	strcpy(grouptitle, myrow[1]);
	
	*pmax_size = atoi(myrow[2]);
	*pmax_user = atoi(myrow[3]);
