...
<BR><BR><P align=right><A href=../data/script/template_dm.xls>%s</A>\n\
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</P>\n\
...
static BOOL upload_ui_info_domain(const char *domainname, char *domain_path);

static BOOL upload_ui_batch_input(const char *domainname,
	DOUBLE_LIST *plist, int *presult);

static BOOL upload_ui_batch_delete(const char *domainname,
	DOUBLE_LIST *plist, int *presult);
...
int upload_ui_run()
{
	...
	if (0 == strcmp(request, "POST")) {
		...
		if (FALSE == upload_ui_info_domain(domainname, domain_path)) {
			upload_ui_error_html(lang_resource_get(
		...
		if (0 == strcasecmp(type, "input")) {
			sprintf(temp_path, "%s/tmp/input.%s.xls", domain_path, session);
		} else if (0 == strcasecmp(type, "delete")) {
			sprintf(temp_path, "/%s/tmp/delete.%s.xls", domain_path, session);
		} else {
			goto POST_ERROR;
		}
		...
		if (0 == strcasecmp(type, "input")) {
			if (FALSE == upload_ui_xls_input(domainname,
				temp_path, &user_list, &result)) {
			...
			if (FALSE == upload_ui_batch_input(domainname, &user_list, &result)) {
				upload_ui_error_html(lang_resource_get(
					g_lang_resource, "ERROR_INTERNAL", language));
				upload_ui_free_ulist(&user_list);
				double_list_free(&user_list);
			...
			if (FALSE == upload_ui_xls_delete(domainname,
				temp_path, &user_list, &result)) {
				double_list_free(&user_list);
				switch (result) {
				case XLS_RESULT_OPENFAIL:
			...
			if (FALSE == upload_ui_batch_delete(domainname,
				&user_list, &result)) {
				upload_ui_error_html(lang_resource_get(
					g_lang_resource, "ERROR_INTERNAL", language));
				upload_ui_free_ulist(&user_list);
		...
		if (NULL == query) {
			...
			upload_ui_main_html(domainname, session);
			return 0;
		}
	} else {
...
RETRYING:
	...
	upload_ui_encode_squote(domainname, temp_name);
	...
	DOUBLE_LIST *plist, int *presult)
{
	...
	if (NULL == (pmysql = mysql_init(NULL)) ||
		...
		system_log_info("[upload_ui]: Failed to connect mysql"
				" service, reason: %s", mysql_error(pmysql));
		locker_client_unlock(lockd);
		return FALSE;
	}

	upload_ui_encode_squote(domainname, temp_buff);
	...
	snprintf(sql_string, 1024, "SELECT id, domain_type, max_size, max_user, "
		"privilege_bits, media FROM domains WHERE domainname='%s'", temp_buff);
	if (0 != mysql_query(pmysql, sql_string) ||
		NULL == (pmyres = mysql_store_result(pmysql))) {
		system_log_info("[upload_ui]: fail to query mysql"
				" server, reason: %s", mysql_error(pmysql));
	...
	for (pnode=double_list_get_head(plist); NULL!=pnode;
		...
		snprintf(sql_string, 1024, "SELECT max_size, domain_id, "
			"address_type, maildir FROM users WHERE username='%s'",
			temp_buff);
		if (0 != mysql_query(pmysql, sql_string) ||
			NULL == (pmyres = mysql_store_result(pmysql))) {
			system_log_info("[upload_ui]: fail to query mysql"
		...
		if (1 == mysql_num_rows(pmyres)) {
			...
			if (domain_id != atoi(myrow[1])) {
				...
				if (ADDRESS_TYPE_NORMAL != atoi(myrow[2])) {
					...
					strcpy(pitem_user->maildir, myrow[3]);
				}
			}
		} else {
	...
	snprintf(sql_string, 1024, "SELECT id, title, privilege_bits"
	...
	for (i=0; i<rows; i++) {
		myrow = mysql_fetch_row(pmyres);
		pitem_group = (GROUP_ITEM*)malloc(sizeof(GROUP_ITEM));
		if (NULL == pitem_group) {
			continue;
		}
		pitem_group->node.pdata = pitem_group;
		pitem_group->group_id = atoi(myrow[0]);
		pitem_group->privilege_bits = atoi(myrow[2]) & 0XFF;
		upload_ui_from_utf8(myrow[1], pitem_group->group_title, 128);

		
		double_list_init(&pitem_group->class_list);
		snprintf(sql_string, 1024, "SELECT id, classname FROM "
			"classes WHERE group_id=%d", pitem_group->group_id);
		if (0 != mysql_query(pmysql, sql_string) ||
		...
		for (j=0; j<rows1; j++) {
			...
			pitem_class->class_id = atoi(myrow1[0]);
			upload_ui_from_utf8(myrow1[1], pitem_class->classname, 128);
			double_list_append_as_tail(&pitem_group->class_list,
				&pitem_class->node);
		}
		mysql_free_result(pmyres1);
		double_list_append_as_tail(&group_list, &pitem_group->node);
	}
	mysql_free_result(pmyres);
	...
	for (pnode=double_list_get_head(plist); NULL!=pnode;
		...
		for (pnode1=double_list_get_head(&group_list); NULL!=pnode1;
			pnode1=double_list_get_after(&group_list, pnode1)) {
			pitem_group = (GROUP_ITEM*)pnode1->pdata;
			if (0 == strcasecmp(pitem_group->group_title,
				pitem_user->group_title)) {
				pitem_user->group_id = pitem_group->group_id;
				pitem_user->privilege_bits = (domain_privilege << 16) |
					(pitem_group->privilege_bits << 8) | 0xFF;
				for (pnode2=double_list_get_head(&pitem_user->class_list);
					NULL!=pnode2; pnode2=double_list_get_after(
					&pitem_user->class_list, pnode2)) {
					pitem_class = (CLASS_ITEM*)pnode2->pdata;
					for (pnode3=double_list_get_head(&pitem_group->class_list);
						NULL!=pnode3; pnode3=double_list_get_after(
						&pitem_group->class_list, pnode3)) {
						pitem_class1 = (CLASS_ITEM*)pnode3->pdata;
		...
		if (0 == pitem_user->group_id) {
			pitem_user->privilege_bits = (domain_privilege << 16)  | 0xFFFF;
		}
		...
		if (FALSE == pitem_user->b_exist) {	
			...
			if (0 != mysql_query(pmysql, sql_string)) {
				system_log_info("[upload_ui]: fail to query mysql server, "
			...
			snprintf(sql_string, 1024, "UPDATE users SET group_id=%d, "
				"title='%s', real_name='%s', nickname='%s', tel='%s', "
				"cell='%s', homeaddress='%s', memo='%s', max_size=%d "
				"WHERE username='%s'", pitem_user->group_id, temp_title,
				temp_real, temp_nick, temp_tel, temp_cell, temp_home,
				temp_memo, pitem_user->size, temp_buff);
			if (0 != mysql_query(pmysql, sql_string)) {
				system_log_info("[upload_ui]: fail to query mysql"
						" server, reason: %s", mysql_error(pmysql));
				pitem_user->result = ITEM_INPUT_SQL;
	...
	locker_client_unlock(lockd);
	mysql_close(pmysql);
	while ((pnode = double_list_get_from_head(&group_list)) != NULL) {
		while ((pnode1 = double_list_get_from_head(&static_cast(GROUP_ITEM *, pnode->pdata)->class_list)) != NULL)
			free(pnode1->pdata);
		double_list_free(&((GROUP_ITEM*)(pnode->pdata))->class_list);
		free(pnode->pdata);
	}
	double_list_free(&group_list);
	...
	DOUBLE_LIST *plist, int *presult)
{
	...
	for (pnode=double_list_get_head(plist); NULL!=pnode;
		...
		snprintf(sql_string, 1024, "SELECT address_type, maildir, "
			"domain_id FROM users WHERE username='%s'", temp_buff);
		if (0 != mysql_query(pmysql, sql_string) ||
			NULL == (pmyres = mysql_store_result(pmysql))) {
			system_log_info("[upload_ui]: fail to query mysql"
					" server, reason: %s", mysql_error(pmysql));
		...
		snprintf(sql_string, 1024, "DELETE FROM "
			"associations WHERE username='%s'", temp_buff);
		if (0 != mysql_query(pmysql, sql_string)) {
			system_log_info("[upload_ui]: fail to query mysql"
					" server, reason: %s", mysql_error(pmysql));
		}
	...
	const char *path, DOUBLE_LIST *plist, int *presult)
{
	...
	for (i=1; i<=pWS->rows.lastrow; i++) {
		...
		if (NULL != row->cells.cell[1].str) {
			if (strlen(row->cells.cell[1].str) > 128) {
		...
		if (pWS->rows.lastcol >= 6) {
			if (NULL != row->cells.cell[6].str) {
				...
				strcpy(pitem->memo, row->cells.cell[6].str);
			}
		}

		if (pWS->rows.lastcol >= 7) {
			if (NULL != row->cells.cell[7].str) {
				if (strlen(row->cells.cell[7].str) >= 128) {
					pitem->result = ITEM_XLS_NICKLEN;
					b_touched = TRUE;
					double_list_append_as_tail(plist, &pitem->node);
					continue;
				}
				strcpy(pitem->nickname, row->cells.cell[7].str);
			}
		}
		...
		if (pWS->rows.lastcol >= 8) {
			if (NULL != row->cells.cell[8].str) {
				...
				strcpy(pitem->tel, row->cells.cell[8].str);
		...
		if (pWS->rows.lastcol >= 9) {
			if (NULL != row->cells.cell[9].str) {
				if (strlen(row->cells.cell[9].str) >= 20) {
					pitem->result = ITEM_XLS_CELLLEN;
					b_touched = TRUE;
					double_list_append_as_tail(plist, &pitem->node);
					continue;
				}
				strcpy(pitem->cell, row->cells.cell[9].str);
		...
		if (pWS->rows.lastcol >= 10) {
			if (NULL != row->cells.cell[10].str) {
				...
				strcpy(pitem->homeaddress, row->cells.cell[10].str);
			}
		}
		
		if (pWS->rows.lastcol >= 11) {
			if (NULL != row->cells.cell[11].str) {
				if (strlen(row->cells.cell[11].str) >= 128) {
					pitem->result = ITEM_XLS_TITLELEN;
					b_touched = TRUE;
					double_list_append_as_tail(plist, &pitem->node);
					continue;
				}
				strcpy(pitem->title, row->cells.cell[11].str);
			}
		}
		
		strcpy(pitem->lang, "en");
		if (pWS->rows.lastcol >= 12) {
			if (NULL != row->cells.cell[12].str &&
		...
		if (NULL != row->cells.cell[4].str) {
			if (strlen(row->cells.cell[4].str) > 127) {
				pitem->result = ITEM_XLS_GROUPTITLELEN;
				b_touched = TRUE;
				double_list_append_as_tail(plist, &pitem->node);
				continue;
			}
			strcpy(pitem->group_title, row->cells.cell[4].str);
		}
		
		if (pWS->rows.lastcol >= 5) {
			ptr1 = row->cells.cell[5].str;
			while (NULL != ptr1) {
				if (' ' == *ptr1) {
					ptr1 ++;
					continue;
				...
				ptr2 = strchr(ptr1, ' ');
				if (NULL == ptr2) {
					ptr2 = row->cells.cell[5].str +
							strlen(row->cells.cell[5].str);
				}
				if (ptr2 - ptr1 >= 32) {
					ptr1 = ptr2 + 1;
					continue;
...
static void upload_ui_error_xls(DOUBLE_LIST *plist)
{
	...
	for (pnode=double_list_get_head(plist); NULL!=pnode;
		...
		case ITEM_XLS_MEMOLEN:
			...
			break;
		case ITEM_XLS_NICKLEN:
			strcpy(result_string, lang_resource_get(
				g_lang_resource, "RESULT_XLS_NICKLEN", language));
			break;
		...
		case ITEM_XLS_HOMELEN:
			strcpy(result_string, lang_resource_get(g_lang_resource,
				"RESULT_XLS_HOMELEN", language));
			break;
		case ITEM_XLS_TITLELEN:
			strcpy(result_string, lang_resource_get(
				g_lang_resource, "RESULT_XLS_TITLELEN", language));
			break;
		case ITEM_XLS_GROUPTITLELEN:
			strcpy(result_string, lang_resource_get(
				g_lang_resource, "RESULT_XLS_GROUPTITLELEN", language));
			break;
		case ITEM_XLS_SIZE:
			strcpy(result_string, lang_resource_get(
...
static void upload_ui_main_html(const char *domainname, const char *session)
{
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_MAIN_5, lang_resource_get(g_lang_resource,
		"MAIN_DOWNLOAD", language), url_buff, lang_resource_get(
		g_lang_resource,"MAIN_BATCH_INPUT", language), domainname,
		session, str_submit, url_buff, lang_resource_get(g_lang_resource,
		"MAIN_BATCH_DELETE", language), domainname, session, str_submit);
	return;
}
