...
function DeleteItem(filename) {location.href='%s?domain=%s&session=%s&delete=' + filename;}\n\
function OpenItem(filename) {window.open('%s?domain=%s&session=%s&open=' + filename, 'TEXT', 'height=200, width=400, toolbar=no, menubar=no, location=no, status=no');}\n\
...
static void list_ui_remove_item(const char *domainname, char *filename);

static void list_ui_item_html(char *filename);

static void list_ui_broadcast_dir(const char *domainname);
...
int list_ui_run()
{
	...
	if (0 == strcmp(request, "POST")) {
		...
		if (FALSE == data_source_info_domain(domainname, domain_path)) {
			list_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_INTERNAL", language));
			return 0;
		}

		snprintf(g_sign_path, 255, "%s/sign", domain_path);
		...
		close(fd);
		list_ui_broadcast_dir(domainname);
		snprintf(command, 1024, "domain_sign.hook add %s", domainname);
		gateway_control_notify(command, NOTIFY_DELIVERY);
		...
		if (NULL == query) {
			...
			if (NULL != (ptr2 = search_string(search_buff, "&open=", len))) {
				...
				list_ui_remove_item(domainname, temp_path);
				list_ui_main_html(domainname, session);
				return 0;
			} else {
...
static void list_ui_remove_item(const char *domainname, char *filename)
{
	...
	if (0 == count) {
		snprintf(command, 1024, "domain_sign.hook remove %s", domainname);
		gateway_control_notify(command, NOTIFY_DELIVERY);
		return;
	}
...
static void list_ui_main_html(const char *domainname, const char *session)
{
	...
	if (0 != stat(g_sign_path, &node_stat)) {
		mkdir(g_sign_path, 0777);
	}
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_MAIN_5, url_buff, domainname, session, url_buff,
		domainname, session);
	printf(url_buff);
	printf(HTML_MAIN_6, domainname, session,
		lang_resource_get(g_lang_resource,"MAIN_CHARSET", language),
	...
	printf(HTML_MAIN_7);
	printf(HTML_TBITEM_FIRST, lang_resource_get(g_lang_resource,"MAIN_CHARSET", language),
		lang_resource_get(g_lang_resource,"MAIN_TYPE", language),
...
static void list_ui_broadcast_dir(const char *domainname)
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/domain_sign/%s", g_mount_path,
			direntp->d_name, domainname);
		...
		while ((direntp1 = readdir(dirp1)) != NULL) {
			...
			sprintf(temp_path, "%s/%s/data/delivery/domain_sign/%s//%s",
				g_mount_path, direntp->d_name, domainname, direntp1->d_name);
			remove(temp_path);
		}
		closedir(dirp1);
		...
		while ((direntp1 = readdir(dirp1)) != NULL) {
			...
			if (node_stat.st_size == read(fd, pbuff, node_stat.st_size)) {
				sprintf(temp_path, "%s/%s/data/delivery/domain_sign/%s/%s",
					g_mount_path, direntp->d_name, domainname,
					direntp1->d_name);
