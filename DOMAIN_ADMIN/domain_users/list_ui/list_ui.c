...
function DeleteItem(username) {if (confirm('%s')) location.href='%s?domain=%s&session=%s&type=remove&username=' + username;}\n\
function EditItem(username) {location.href='%s?domain=%s&session=%s&type=edit&username=' + username;}\n\
...
<TR><TD></TD><TD vAlign=center>%s</TD><TD vAlign=center><SPAN>\n\
<SELECT name=group_id><OPTION value=-1 selected>%s</OPTION>\n\
<OPTION value=0>%s</OPTION>"

#define HTML_SEARCH_7	\
"</SELECT></TD></TR><TR><TD></TD><TD vAlign=center>%s</TD><TD vAlign=center>\n\
...
<TR><TD></TD><TD vAlign=center>%s</TD><TD vAlign=center><SPAN>\n\
<SELECT name=group_id><OPTION value=0 selected>%s</OPTION>\n"
...
apos=aliasform.aliasname.value.indexOf('@');\n\
var regstr=/^[\\.\\-_A-Za-z0-9]+@([-_A-Za-z0-9]+\\.)+[A-Za-z0-9]{2,6}$/;\n\
if (0 == apos) {\n\
	alert('%s');\n\
	return false;\n\
} else if (apos > 0) {\n\
	if (aliasform.aliasname.value.substring(apos, \n\
	aliasform.aliasname.value.length).toLowerCase() != '@%s'.toLowerCase()) {\n\
		alert('%s');\n\
		return false;\n\
	}\n\
	if (!regstr.test(aliasform.aliasname.value)) {\n\
		alert('%s');\n\
		return false;\n\
	}\n\
} else {\n\
	var aliasstr = aliasform.aliasname.value + '@%s';\n\
	if (!regstr.test(aliasstr)) {\n\
		alert('%s');\n\
		return false;\n\
	}\n\
}\n\
...
<TR><TD></TD><TD vAlign=ceneer>%s</TD><TD vAlign=center><SPAN>\n\
<SELECT name=group_id>\n"

#define HTML_EDIT_7_1	\
"</SELECT></SPAN></TD></TR><TR class=%s><TD></TD><TD vAlign=center>%s</TD>\n\
...
static void list_ui_search_html(const char *domainname, const char *session);

static void list_ui_add_html(const char *domainname, const char *session);

static void list_ui_alias_html(const char *domainname, const char *session);

static void list_ui_add_ok_html(const char *domainname,
	const char *session, const char *username);

static void list_ui_remove_ok_html(const char *domainname,
	const char *session, int page_index);

static void list_ui_alias_ok_html(const char *domainname,
	const char *session);

static void list_ui_edit_ok_html(const char *domainname,
	const char *session, int page_index);
...
static void list_ui_edit_html(const char *domainname,
	const char *session, const char *username);

static void list_ui_result_html(const char *domainname, const char *session,
	const char *username, int size_min, int size_max, const char *title,
	const char *real_name, const char *nickname, const char *tel,
	const char *cell, const char *homeaddress, const char *memo, int group_id,
	time_t create_min, time_t create_max, int address_status,
	int address_type);

static void list_ui_page_html(const char *domainname, const char *session,
	int page_index);

static void list_ui_page_ex_html(const char *domainname, const char *session,
	int page_index);
...
static unsigned int list_ui_cache_result(const char *domainname,
	...
	const char *memo, int group_id, time_t create_min, time_t create_max,
	int address_status, int address_type);

static BOOL list_ui_cache_retrieve(const char *domainname, const char *session,
	const char *username, USER_ITEM *pitem);

static unsigned int list_ui_cache_edit(const char *domainname,
	const char *session, const char *username, int max_size,
	const char *title, const char *real_name, const char *nickname,
	const char *tel, const char *cell, const char *homeaddress,
	const char *memo, int group_id, int privilege_bits, int address_status);

static unsigned int list_ui_cache_delete(const char *domainname,
	const char *session, const char *username);
...
int list_ui_run()
{
	...
	if (0 == strcmp(request, "POST")) {
		...
		if (NULL == pat) {
			strcat(username, "@");
			strcat(username, domainname);
		} else {
			if (pat == username || 0 != strcasecmp(pat + 1, domainname)) {
				goto POST_ERROR;
			}
		}
		if (0 == strcasecmp(type, "search")) {
			...
			list_ui_result_html(domainname, session, username, size_min,
				size_max, title, real_name, nickname, tel, cell, homeaddress,
				memo, group_id, create_min, create_max, address_status,
				address_type);
			return 0;
			...
			if (0 == strcasecmp(type, "add")) {
				...
				if (TRUE == data_source_check_domain_migration(domainname,
				...
				case ADD_RESULT_OK:
					...
					list_ui_add_ok_html(domainname, session, username);
					break;
				case ADD_RESULT_NODOMAIN:
					...
					list_ui_add_error_html(lang_resource_get(g_lang_resource,
				...
				case ADD_RESULT_DOMAINNOTMAIN:
					...
					break;
				case ADD_RESULT_SIZEFULL:
					list_ui_free_dir(FALSE, maildir);
					if ('\0' != media_area[0]) {
						list_ui_free_dir(TRUE, mediadir);
					}
					list_ui_add_error_html(lang_resource_get(g_lang_resource,
						"ADD_ERROR_SIZEFULL", language));
					break;
				case ADD_RESULT_USERFULL:
					list_ui_free_dir(FALSE, maildir);
					if ('\0' != media_area[0]) {
						list_ui_free_dir(TRUE, mediadir);
					}
					list_ui_add_error_html(lang_resource_get(g_lang_resource,
						"ADD_ERROR_USERFULL", language));
					break;
				case ADD_RESULT_NOGROUP:
					...
					break;
				case ADD_RESULT_GROUPERR:
					list_ui_free_dir(FALSE, maildir);
					if ('\0' != media_area[0]) {
						list_ui_free_dir(TRUE, mediadir);
					}
					list_ui_add_error_html(lang_resource_get(g_lang_resource,
						"ADD_ERROR_GROUPERR", language));
					break;
				...
				if (FALSE == list_ui_cache_retrieve(domainname,
					session, username, &temp_item)) {
					list_ui_edit_error_html(lang_resource_get(g_lang_resource,
						"EDIT_ERROR_NOCACHE", language));
					return 0;
				}
				...
				list_ui_to_utf8(homeaddress, new_home, 128);
				list_ui_to_utf8(memo, new_memo, 128);
				if (FALSE == data_source_edit_user(username, encrypt_pw,
					new_title, new_name, new_nick, new_tel, new_cell,
					new_home, new_memo, group_id, max_size, privilege_bits,
				...
				case EDIT_RESULT_OK:
					...
					if (NULL != pcollection) {
						...
						for (data_source_collect_begin(pcollection);
							...
							list_ui_cache_edit(domainname, session,
								pitem->username, max_size, title, real_name,
								nickname, tel, cell, homeaddress, memo,
								group_id, privilege_bits, address_status);
						}
					}
					data_source_collect_free(pcollection);
				
					page_index = list_ui_cache_edit(domainname, session,
					...
					list_ui_edit_ok_html(domainname, session, page_index);
					break;
				...
				case EDIT_RESULT_NOEXIST:
					...
					break;
				case EDIT_RESULT_SIZEFULL:
					list_ui_edit_error_html(lang_resource_get(
						g_lang_resource, "EDIT_ERROR_SIZEFULL", language));
					break;
			...
			if (NULL == pvalue || strlen(pvalue) >= sizeof(aliasname)) {
				goto POST_ERROR;
			}
			strcpy(aliasname, pvalue);
			pat = strchr(aliasname, '@');
			if (NULL == pat) {
				if (strlen(aliasname) + 1 + strlen(aliasname) >= 128) {
					goto POST_ERROR;
				}
				strcat(aliasname, "@");
				strcat(aliasname, domainname);
			} else {
				if (pat == aliasname || 0 != strcasecmp(pat + 1, domainname)) {
					goto POST_ERROR;
				}
			}
			
			if (FALSE == data_source_add_alias(username, aliasname, &result)) {
				list_ui_error_html(lang_resource_get(
					g_lang_resource, "ERROR_INTERNAL", language));
				return 0;
			}
			...
			case ALIAS_RESULT_OK:
				list_ui_alias_ok_html(domainname, session);
				break;
		...
		type = request_parser_get(pparser, "type");
		if (NULL == type) {
			if (FALSE == data_source_num_user(domainname, &total_users)) {
				list_ui_error_html(lang_resource_get(
					g_lang_resource, "ERROR_INTERNAL", language));
				return 0;
			}
			if (total_users <= 10000) {
				list_ui_result_html(domainname, session, "", 0,  0,
					"", "", "", "", "", "", "", -1, 0, 0, -1, -1);
			} else {
				list_ui_search_html(domainname, session);
			}
			return 0;
		}
		pvalue = request_parser_get(pparser, "username");
		if (NULL != pvalue) {
			...
			if (0 == strcasecmp(type, "edit")) {
				list_ui_edit_html(domainname, session, username);
				...
				if (FALSE == b_alias) {
					...
					for (data_source_collect_begin(pcollection);
						...
						data_source_collect_forward(pcollection)) {
						pitem = data_source_collect_get_value(pcollection);
						list_ui_cache_delete(domainname,
				...
				page_index = list_ui_cache_delete(
					domainname, session, username);
				list_ui_remove_ok_html(domainname, session, page_index);
			} else {
				goto GET_ERROR;
			}
			return 0;
		...
		if (NULL != pvalue) {
			...
			if (0 == strcasecmp(type, "paging")) {
				list_ui_page_html(domainname, session, page_index);
			} else if (0 == strcasecmp(type, "paging-ex")) {
				list_ui_page_ex_html(domainname, session, page_index);
			} else {
				goto GET_ERROR;
			}
			return 0;
		...
		if (0 == strcasecmp(type, "add")) {
			...
			list_ui_alias_html(domainname, session);
...
static void list_ui_add_ok_html(const char *domainname, const char *session,
	const char *username)
{
	char *language;
	char url_buff[1024];
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_ADD_OK_5, lang_resource_get(g_lang_resource,"ADDING_OK", language), url_buff,
		domainname, session, lang_resource_get(g_lang_resource,"OK_LABEL", language));
}

static void list_ui_edit_ok_html(const char *domainname, const char *session,
	int page_index)
{
	char *language;
	char url_buff[1024];
...
static void list_ui_alias_ok_html(const char *domainname, const char *session)
{
	char *language;
	char url_buff[1024];
	...
	const char *session, int page_index)
{
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_PAGE_OK_5, lang_resource_get(g_lang_resource,"REMOVE_OK", language),
		url_buff, domainname, session, page_index,
		lang_resource_get(g_lang_resource,"OK_LABEL", language));
}
...
static void list_ui_add_html(const char *domainname, const char *session)
{
	...
	if (FALSE == data_source_get_groups(domainname, pcollection) ||
		FALSE == data_source_get_domain_privilege(domainname, &privilege)) {
		data_source_collect_free(pcollection);
		list_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_INTERNAL", language));
		return;
	}
	...
	sprintf(url_search, "%s?domain=%s&session=%s&type=search", url_buff,
		domainname, session);
	sprintf(url_alias, "%s?domain=%s&session=%s&type=alias", url_buff,
		domainname, session);
	sprintf(url_list, "%s?domain=%s&session=%s&type=list", url_buff,
		domainname, session);
	printf("Content-Type:text/html;charset=%s\n\n",
		lang_resource_get(g_lang_resource,"CHARSET", language));
	printf(HTML_COMMON_1);
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_ADD_5, url_search, lang_resource_get(g_lang_resource,"SEARCH_LABEL",
	...
	if (privilege & DOMAIN_PRIVILEGE_EXTPASSWD) {
		...
		str_default = lang_resource_get(g_lang_resource,"PASSWORD_AGING_DEFAULT", language);
	} else {
	...
	langs = (char*)lang_resource_get(g_lang_resource, "LANGUAGE_ITEMS", language);
	ptr = langs;
	b_first = FALSE;
	while (TRUE) {
		ptr2 = strchr(ptr, '|');
	...
	printf(HTML_ADD_7, lang_resource_get(g_lang_resource,"MAIN_ADDRESS_STATUS", language),
		...
		lang_resource_get(g_lang_resource,"FREE_GROUP", language));

	for (data_source_collect_begin(pcollection);
		!data_source_collect_done(pcollection);
		data_source_collect_forward(pcollection)) {
		pitem = (USER_ITEM*)data_source_collect_get_value(pcollection);
		list_ui_from_utf8(pitem->group_title, temp_title, 128);
		printf(SELECT_OPTION, pitem->group_id, temp_title);
	}
	
	data_source_collect_free(pcollection);

	printf(HTML_ADD_8, lang_resource_get(g_lang_resource,"MAIN_MAX_SIZE", language),
	...
	printf(HTML_ADD_9, lang_resource_get(g_lang_resource,"ADD_LABEL", language),
		...
		lang_resource_get(g_lang_resource,"MSGERR_USERNAME", language), domainname,
...
static void list_ui_alias_html(const char *domainname, const char *session)
{
	...
	printf(HTML_ALIAS_5, url_search, lang_resource_get(g_lang_resource,"SEARCH_LABEL",
		...
		url_list, lang_resource_get(g_lang_resource,"WHOLE_LIST", language));
	printf(url_buff);
	printf(HTML_ALIAS_6, domainname, session,
		lang_resource_get(g_lang_resource,"MAIN_USERNAME", language),
		lang_resource_get(g_lang_resource,"MAIN_ALIAS", language),
		lang_resource_get(g_lang_resource,"OK_LABEL", language),
		lang_resource_get(g_lang_resource,"MSGERR_USERNAME", language),
		lang_resource_get(g_lang_resource,"MSGERR_USERNAME_LEN", language), domainname,
		lang_resource_get(g_lang_resource,"MSGERR_DOMAINDIFF", language),
		lang_resource_get(g_lang_resource,"MSGERR_ALIAS", language),
		lang_resource_get(g_lang_resource,"MSGERR_ALIAS_LEN", language),
		lang_resource_get(g_lang_resource,"MSGERR_ALIAS", language), domainname,
		lang_resource_get(g_lang_resource,"MSGERR_DOMAINDIFF", language),
		lang_resource_get(g_lang_resource,"MSGERR_ALIASERR", language), domainname,
		lang_resource_get(g_lang_resource,"MSGERR_ALIASERR", language),
		lang_resource_get(g_lang_resource,"COMPANY_INFORMATION", language));
}
	...
	const char *session, const char *username)
{
	...
	if (FALSE == list_ui_cache_retrieve(domainname, session, username,
		&temp_item)) {
		list_ui_error_html(lang_resource_get(
			g_lang_resource, "ERROR_REQUEST", language));
		return;
	...
	if (ADDRESS_TYPE_ALIAS == temp_item.address_type) {
		...
		sprintf(prompt, lang_resource_get(g_lang_resource,
					"PROMPT_ALIAS", language), temp_user);
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_EDIT_5, lang_resource_get(g_lang_resource,"BACK_TO_LIST", language), prompt);
	printf(url_buff);
	if (privilege & DOMAIN_PRIVILEGE_EXTPASSWD) {
		strcpy(option_domain, OPTION_ENABLED);
		str_default = lang_resource_get(g_lang_resource,"PASSWORD_AGING_DEFAULT", language);
	...
	printf(HTML_EDIT_6, domainname, session,
			...
			option_enabled, lang_resource_get(g_lang_resource,"STATUS_NORMAL",
			language), option_disabled, lang_resource_get(g_lang_resource,"STATUS_SUSPEND",
			language), lang_resource_get(g_lang_resource,"MAIN_GROUP", language));
	
	if (0 == temp_item.group_id) {
		printf(SELECT_OPTION_EX, 0, lang_resource_get(g_lang_resource,"FREE_GROUP", language));
	} else {
		printf(SELECT_OPTION, 0, lang_resource_get(g_lang_resource,"FREE_GROUP", language));
	}
	...
	for (data_source_collect_begin(pcollection);
		...
		pitem = (USER_ITEM*)data_source_collect_get_value(pcollection);
		list_ui_from_utf8(pitem->group_title, temp_title, 128);
		if (temp_item.group_id == pitem->group_id) {
			printf(SELECT_OPTION_EX, pitem->group_id, temp_title);
		} else {
			printf(SELECT_OPTION, pitem->group_id, temp_title);
		}
	}
	...
	if (mb_size >= 1024*1024) {
		printf(HTML_EDIT_7_1, class_size, lang_resource_get(
			...
			temp_item.cell,
			lang_resource_get(g_lang_resource,"MAIN_HOMEADDRESS", language),
			temp_item.homeaddress,
			lang_resource_get(g_lang_resource,"MAIN_MEMO", language),
			temp_item.memo);
	} else {
		printf(HTML_EDIT_7_2, class_size, lang_resource_get(g_lang_resource,
			"MAIN_MAX_SIZE", language), temp_item.max_size/1024, (int)(mb_size/1024),
			lang_resource_get(g_lang_resource,"MAIN_USER_TITLE", language),
			temp_item.title,
			lang_resource_get(g_lang_resource,"MAIN_REAL_NAME", language),
			temp_item.real_name,
	...
	strftime(create_buff, 32, lang_resource_get(
		g_lang_resource, "DATE_FORMAT", language),
		&temp_tm);

	printf(HTML_EDIT_8, lang_resource_get(
		g_lang_resource, "MAIN_CREATING_DAY",
		language), create_buff);
...
static void list_ui_search_html(const char *domainname, const char *session)
{
	...
	sprintf(url_alias, "%s?domain=%s&session=%s&type=alias", url_buff,
		domainname, session);
	sprintf(url_list, "%s?domain=%s&session=%s&type=list", url_buff, domainname,
		session);
	printf("Content-Type:text/html;charset=%s\n\n",
		lang_resource_get(g_lang_resource,"CHARSET", language));
	printf(HTML_COMMON_1);
	...
	printf(HTML_SEARCH_6, domainname, session,
		lang_resource_get(g_lang_resource,"MAIN_USERNAME", language),
		lang_resource_get(g_lang_resource,"MAIN_ADDRESS_STATUS", language),
	...
	printf(HTML_SEARCH_7, lang_resource_get(g_lang_resource,"MAIN_MAX_SIZE", language),
		lang_resource_get(g_lang_resource,"MAIN_USER_TITLE", language),
		lang_resource_get(g_lang_resource,"MAIN_REAL_NAME", language),
		lang_resource_get(g_lang_resource,"MAIN_NICKNAME", language),
		lang_resource_get(g_lang_resource,"MAIN_TEL", language),
	...
	printf(HTML_SEARCH_8, lang_resource_get(g_lang_resource,"MAIN_CREATING_DAY", language),
	...
	const char *session, const char *username)
{
	...
	if (NULL == pat) {
		strcat(temp_address, "@");
		strcat(temp_address, domainname);
	}
	page_index = 1;
	if (FALSE == data_source_get_domain_homedir(domainname, domain_path)) {
		return page_index;
	}
	...
	if (-1 != fd) {
		...
		while (sizeof(temp_item) == read(fd, &temp_item, sizeof(temp_item))) {
			if (0 == strcasecmp(temp_item.username, temp_address)) {
				...
				if (temp_item.group_id != group_id) {
					temp_item.group_id = group_id;
					data_source_get_grouptitle(group_id, group_title);
					list_ui_from_utf8(group_title, temp_item.group_title, 128);
				}
...
static BOOL list_ui_cache_retrieve(const char *domainname,
	...
	int address_status, int address_type)
{
	...
	if ('\0' == title[0]) {
		...
		list_ui_to_utf8(title, temp_utitle, 128);
		putitle = temp_utitle;
	}
	...
	if (FALSE == data_source_query(domainname, pusername, putitle, preal_name,
		pnickname, ptel, pcell, phomeaddress, pmemo, group_id, size_min,
		size_max, create_min, create_max, address_status, address_type,
		pcollection)) {
		return 0;
	}
	time(&cur_time);
	strcat(domain_path, "/tmp");
	dirp = opendir(domain_path);
	if (NULL != dirp) {
		while (direntp = readdir(dirp)) {
			...
			if (0 != strncmp(direntp->d_name, "domain_users.", 13)) {
				continue;
			}
			sprintf(temp_path, "%s/%s", domain_path, direntp->d_name);
	...
	page_index = 1;
	sprintf(temp_path, "%s/domain_users.%s", domain_path, session);
	...
	if (-1 != fd) {
		...
		for (data_source_collect_begin(pcollection);
			...
			list_ui_from_utf8(pitem->title, temp_utitle, 128);
			strcpy(pitem->title, temp_utitle);
			...
			list_ui_from_utf8(pitem->group_title, temp_title, 128);
			strcpy(pitem->group_title, temp_title);
...
static void list_ui_result_html(const char *domainname,
	...
	int address_type)
{
	...
	language = getenv("HTTP_ACCEPT_LANGUAGE");
	page_index = list_ui_cache_result(domainname, session,
			username, size_min, size_max, title, real_name,
			nickname, tel, cell, homeaddress, memo, group_id,
			create_min, create_max, address_status, address_type);
	if (0 == page_index) {
		list_ui_error_html(lang_resource_get(
			g_lang_resource, "ERROR_INTERNAL", language));
		return;
	...
	int page_index)
{
	...
	sprintf(url_search, "%s?domain=%s&session=%s&type=search", url_buff,
		domainname, session);
	sprintf(url_add, "%s?domain=%s&session=%s&type=add", url_buff,
		domainname, session);
	sprintf(url_alias, "%s?domain=%s&session=%s&type=alias", url_buff,
		domainname, session);
	sprintf(url_paging_ex, "%s?domain=%s&session=%s&type=paging-ex&index=%d",
		url_buff, domainname, session, page_index);
	
	sprintf(url_paging_first, "%s?domain=%s&session=%s&type=paging&index=1",
		url_buff, domainname, session);
	sprintf(url_paging_last, "%s?domain=%s&session=%s&type=paging&index=%d",
		url_buff, domainname, session, pages);
	...
	if (page_index < pages) {
		sprintf(url_paging_next, "%s?domain=%s&session=%s&type=paging&index=%d",
			url_buff, domainname, session, page_index + 1);
	...
	if (page_index > 1) {
		sprintf(url_paging_prev, "%s?domain=%s&session=%s&type=paging&index=%d",
			url_buff, domainname, session, page_index - 1);
	...
	printf(HTML_TBITEM_FIRST, lang_resource_get(g_lang_resource,"MAIN_USERNAME", language),
		lang_resource_get(g_lang_resource,"MAIN_REAL_NAME", language),
		lang_resource_get(g_lang_resource,"MAIN_GROUP", language),
	...
	for (i=(page_index-1)*ITEMS_PER_PAGE+1; i<=num; i++) {
		...
		if (0 == temp_item.group_id) {
			strcpy(group_title, lang_resource_get(g_lang_resource,"FREE_GROUP", language));
		} else {
			strcpy(group_title, temp_item.group_title);
		}
		if (RECORD_STATUS_DELETED == temp_item.address_status) {
			printf(HTML_TBITEM_DELETED, temp_item.username,
				temp_item.real_name, group_title, temp_item.max_size/1024);
		} else if (RECORD_STATUS_SUSPEND == temp_item.address_status) {
			printf(HTML_TBITEM, CSS_ITEM_SUSPEND, temp_item.username,
				temp_item.real_name, group_title, temp_item.max_size/1024,
				temp_user, lang_resource_get(g_lang_resource,"EDIT_LABEL", language),
				temp_user, lang_resource_get(g_lang_resource,"DELETE_LABEL", language));
		} else {
			if (0 == rows%2) {
				printf(HTML_TBITEM, CSS_ITEMEVEN, temp_item.username,
					temp_item.real_name, group_title, temp_item.max_size/1024,
					temp_user, lang_resource_get(g_lang_resource,"EDIT_LABEL",
					language), temp_user, lang_resource_get(g_lang_resource,"DELETE_LABEL",
					language));
			} else {
	...
	int page_index)
{
	...
	if (FALSE == data_source_get_domain_homedir(
		domainname, domain_path)) {
		list_ui_error_html(lang_resource_get(
			g_lang_resource, "ERROR_INTERNAL", language));
		return;
	}
	sprintf(temp_path, "%s/tmp/domain_users.%s",
		domain_path, session);
	if (0 != stat(temp_path, &node_stat)) {
		list_ui_error_html(lang_resource_get(
			g_lang_resource, "ERROR_REQUEST", language));
	...
	printf(HTML_RESULT_5_1, lang_resource_get(g_lang_resource,"CONFIRM_DELETE", language),
		url_buff, domainname, session, url_buff, domainname, session,
		url_search, lang_resource_get(g_lang_resource,"SEARCH_LABEL", language),
		url_add, lang_resource_get(g_lang_resource,"ADD_LABEL", language),
		url_alias, lang_resource_get(g_lang_resource,"ALIAS_LABEL", language));
	...
	for (i=(page_index-1)*ITEMS_PER_PAGE+1; i<=num; i++) {
		...
		list_ui_encode_squote(temp_item.username, temp_user);
		if (0 == temp_item.group_id) {
			strcpy(group_title, lang_resource_get(
				g_lang_resource, "FREE_GROUP", language));
		} else {
			strcpy(group_title, temp_item.group_title);
		}
		mb_size = 0;
		...
		if (RECORD_STATUS_DELETED == temp_item.address_status) {
			...
			printf(HTML_TBITEM_EX, CSS_ITEM_SUSPEND, temp_item.username,
				temp_item.real_name, group_title, temp_item.max_size/1024,
				(double)mb_size/(1024*1024), temp_user,
				lang_resource_get(g_lang_resource,"EDIT_LABEL", language), temp_user,
...
static BOOL list_ui_allocate_dir(const char *media_area, char *path_buff)
{
	...
	if (0 == double_list_get_nodes_num(&temp_list)) {
		...
		system_log_info("[list_ui]: cannot find a available data area "
			"for user");
