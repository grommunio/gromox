...
#define HTML_SEARCH_7	" name=group>\n<INPUT type=hidden value="
...
return false;\n}\n}\n\
var b_search = false;\n\
var apos=searchpattern.from.value.indexOf('@');\n\
if (apos > 0 && searchpattern.from.value.substring(apos,\n\
	searchpattern.from.value.length).toLowerCase() == '@%s'.toLowerCase()) {\n\
	b_search = true;\n\
}\n\
apos = searchpattern.rcpt.value.indexOf('@');\n\
if (apos > 0 && searchpattern.rcpt.value.substring(apos,\n\
	searchpattern.rcpt.value.length).toLowerCase() == '@%s'.toLowerCase()) {\n\
	b_search = true;\n\
}\n\
if (b_search == false) {\n\
	alert('%s');\n\
...
static void ui_center_search_html(const char *group, const char *session);

static void ui_center_result_html(const char *group, const char *session,
...
int ui_center_run()
{
	...
	if (0 == strcmp(request, "POST")) {
		...
		b_search = FALSE;
		pdomain = strchr(group, '@') + 1;

		if ('\0' != tmp_from[0]) {
			pdomain1 = strchr(tmp_from, '@');
			if (NULL == pdomain1) {
				pdomain1 = tmp_from;
			} else {
				pdomain1 ++;
		...
		data_source_info_group(group, &privilege_bits);

		if ((privilege_bits&GROUP_PRIVILEGE_LOG) == 0) {
			ui_center_error_html(lang_resource_get(g_lang_resource,"ERROR_PRIVILEGE",
				language));
			return 0;
		}
		...
		ui_center_result_html(group, session, tmp_ip, tmp_from, tmp_rcpt,
			start, end);
		return 0;
		...
		if (NULL == query) {
			...
			ui_center_search_html(group, session);
			return 0;
		}
	} else {
...
static void ui_center_search_html(const char *group, const char *session)
{
	...
	printf(HTML_SEARCH_16, pdomain, pdomain,
		lang_resource_get(g_lang_resource,"MSGERR_CONDITION_ERROR", language));
	...
	time_t end_point)
{
	...
	if (NULL != strchr(from, '@')) {
		if (TRUE == data_source_check_group(group, from, &b_result) &&
			TRUE == b_result) {
			b_search = TRUE;
		}
	}
	...
	if (NULL != strchr(rcpt, '@')) {
		if (TRUE == data_source_check_group(group, rcpt, &b_result) &&
			TRUE == b_result) {
			b_search = TRUE;
		}
	}
	...
	if (TRUE == b_search) {
		if (FALSE == search_engine_search(pdomain, ip, from, rcpt, start_point,
	...
	printf(url_buff);
	printf("?group=%s&session=%s", group, session);
