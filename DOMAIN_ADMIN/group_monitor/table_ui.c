...
function DeleteItem(passive, dest) {location.href='%s?group=%s&session=%s&passive=' + passive + '&dest=' + dest;}\n\
...
static void	table_ui_modify_list(const char *group, const char *passive,
	int type,const char *dest);

static void table_ui_remove_item(const char *group, const char *passive,
	const char *dest);
...
static void table_ui_broadcast_list(const char *group);
...
int table_ui_run()
{
	...
	if (g_lang_resource == nullptr) {
		system_log_info("[table_ui]: failed to init language resource");
		return -1;
	}
	...
	if (0 == strcmp(request, "POST")) {
		...
		if (NULL == query) {
			...
			if (NULL == ptr2) {
				...
				if (FALSE == session_client_check(group, session)) {
				...
				*pat = '\0';
				if (FALSE == data_source_info_group(group, &privilege_bits,
					g_group_path) || '\0' == g_group_path[0]) {
					table_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_INTERNAL",
						language));
					return 0;
				}
				if ((privilege_bits&GROUP_PRIVILEGE_MONITOR) == 0 ||
					(privilege_bits&GROUP_PRIVILEGE_DOMAIN_MONITOR) == 0) {
					table_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_PRIVILEGE",
						language));
					return 0;
				}
				strcat(g_group_path, "/");
				strcat(g_group_path, fake_group);
				table_ui_main_html(group, session);
				return 0;
			}
			if (ptr2 <= ptr1 || ptr2 - ptr1 > 255) {
				system_log_info("[table_ui]: query string of GET "
			...
			if (NULL == ptr2) {
				...
				*pat = '\0';
				...
				table_ui_remove_item(group, temp_passive, dest);
				table_ui_main_html(group, session);
				return 0;
			}
			if (ptr2 <= ptr1 || ptr2 - ptr1 > 256) {
			...
			*pat = '\0';
			...
			table_ui_modify_list(group, temp_passive, type, dest);
			table_ui_main_html(group, session);
			return 0;
		}
	} else {
...
static void table_ui_main_html(const char *group, const char *session)
{
	...
	printf(HTML_COMMON_4);
	printf(g_logo_link);
	printf(HTML_MAIN_5, url_buff, group, session);
	printf(url_buff);
	printf(HTML_MAIN_6, group, session,
	...
	int type, const char *dest)
{
	...
	if (NULL == pfile) {
		...
		close(fd);
		table_ui_broadcast_list(group);
		snprintf(command_line, 1024, "group_monitor.hook add %s", group);
	...
	const char *dest)
{
	...
	if (0 == stat(temp_path, &node_stat) && 0 == node_stat.st_size) {
		remove(temp_path);
		snprintf(command_line, 1024, "group_monitor.hook remove %s", group);
	} else {
		link(temp_path, list_path);
		remove(temp_path);
		table_ui_broadcast_list(group);
...
static void table_ui_broadcast_list(const char *group)
{
	...
	if (NULL == dirp){
		system_log_info("[table_ui]: failed to open directory %s: %s",
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/group_monitor/%s.txt",
			g_mount_path, direntp->d_name, group);
