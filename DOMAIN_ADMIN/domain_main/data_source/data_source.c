...
RETRYING:
	...
	sprintf(sql_string, "SELECT password, domain_status, domain_type,"
			" homedir FROM domains WHERE domainname='%s'", temp_name);
	...
	if (1 == mysql_num_rows(pmyres)) {
		...
		*pdomain_status = atoi(myrow[1]);
		*pdomain_type = atoi(myrow[2]);
		strcpy(path_buff, myrow[3]);
		*presult = TRUE;
	}
