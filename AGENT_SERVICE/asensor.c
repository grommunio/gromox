...
int main(int argc, const char **argv)
{
	...
	pconfig = config_file_init2(opt_config_file, config_default_path("asensor.cfg"));
	...
	if (NULL == str_value) {
		...
		g_sensor_port = atoi(str_value);
		if (g_sensor_port <= 0) {
			g_sensor_port = 11111;
		}
	}
	printf("[system]: sensor listen port %d\n", g_sensor_port);


	str_value = config_file_get_value(pconfig, "SENSOR_CONNECTION_NUM");
	if (NULL == str_value) {
		g_conn_num = 10;
		config_file_set_value(pconfig, "SENSOR_CONNECTION_NUM", "10");
	} else {
	...
	printf("[system]: sensor connection number is %d\n", g_conn_num);
    ...
    printf("[system]: ASENSOR is now running\n");
...
static int connect_sensor()
{
    ...
    servaddr.sin_port = htons(g_sensor_port);
    inet_pton(AF_INET, g_sensor_ip, &servaddr.sin_addr);
