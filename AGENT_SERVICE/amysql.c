...
static MYSQL_CONN *get_connection(void);
...
int main(int argc, const char **argv)
{
	...
	pconfig = config_file_init2(opt_config_file, config_default_path("amysql.cfg"));
	...
	if (NULL == str_value) {
		g_conn_num = 5;
		config_file_set_value(pconfig, "MYSQL_CONNECTION_NUM", "5");
	} else {
	...
	if (str_value == NULL) {
		HX_strlcpy(CS_PATH, PKGRUNDIR "/amysql.sock", sizeof(CS_PATH));
