...
static tproc_status ps_cmd_processing(imap_context &ctx)
{
	...
	while (true) {
		...
		if (pcontext->sched_stat == isched_stat::idling) {
			...
			if (argc != 1 || strcasecmp(argv[0].c_str(), "DONE") != 0) {
		...
		switch (imap_parser_dispatch_cmd(argv, ctx)) {
		case DISPATCH_CONTINUE:
			pcontext->command_len = 0;
			return tproc_status::literal_processing;
		case DISPATCH_BREAK:
...
static void wrdat_bin(imap_context &ctx, char *last_line)
{
	/*
	 * FETCH BINARY (RFC 3516): "file|coff|clen|enc|poff|plen". The raw
	 * bytes are cut from the eml and CTE-decoded here, one marker at a
	 * time, so no decoded blob outlives its own literal8 ("~{n}"). The
	 * <poff.plen> partial applies to the decoded content.
	 */
	char *fld[6]{};
	fld[0] = last_line + 7;
	unsigned int nfld = 1;
	...
	if (!ok) {
		strcpy(&ctx.write_buff[ctx.write_length], "NIL");
		ctx.write_length += 3;
		return;
	}
	...
	ctx.wrdat_offset = 0;
	ctx.literal_len = ctx.wrdat_content->size();
	ctx.current_len = 0;
	ctx.write_length += sprintf(&ctx.write_buff[ctx.write_length], "~{%u}\r\n", ctx.literal_len);
	int len = MAX_LINE_LENGTH - ctx.write_length;
	if (len > ctx.literal_len)
		len = ctx.literal_len;
	memcpy(&ctx.write_buff[ctx.write_length], ctx.wrdat_content->data(), len);
	ctx.wrdat_offset += len;
	ctx.current_len += len;
	ctx.write_length += len;
	if (ctx.literal_len == len) {
		ctx.wrdat_content = nullptr;
		ctx.wrdat_backing.reset();
		ctx.literal_len = 0;
		ctx.current_len = 0;
...
static int imap_parser_dispatch_cmd2(std::span<std::string> argv,
    imap_context &ctx)
{
	assert(argv.size() >= 2);
	auto pcontext = &ctx;
	char reply_buff[1024];
	static constexpr std::pair<const char *, int (*)(std::span<std::string>, imap_context &)> proc[] = {
		{"APPEND", icp_append},
	...
	if (strcasecmp(argv[1].c_str(), "UID") == 0) {
		...
		auto it = std::lower_bound(std::begin(proc), std::end(proc), argv[1].c_str(), scmp);
		if (it != std::end(proc) && strcasecmp(argv[1].c_str(), it->first) == 0)
			return it->second(argv, *pcontext);
	}

	auto imap_reply_str = resource_get_imap_code(1800, 1);
	auto string_length = gx_snprintf(reply_buff, std::size(reply_buff), "%s %s", argv[0].c_str(), imap_reply_str);
	pcontext->connection.write(reply_buff, string_length);
	return DISPATCH_CONTINUE;
}

static int imap_parser_dispatch_cmd(std::span<std::string> argv, imap_context &ctx) try
{
	assert(argv.size() >= 2);
	/*
