...
static BOOL table_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[table_ui]: fail to get SERVER_NAME or "
...
static void table_ui_main_html(const char *session)
{
	...
	printf(HTML_MAIN_6, session,
		...
		lang_resource_get(g_lang_resource,"MAIN_MX", language),
		lang_resource_get(g_lang_resource,"MAIN_A", language),
	...
	for (i=0; i<item_num; i++) {
		if (0 == strcmp("MX", pitem[i].type)) {
			type = TYPE_MX;
			strcpy(item_type, lang_resource_get(g_lang_resource,"MAIN_MX", language));
		} else {
			type = TYPE_A;
			strcpy(item_type, lang_resource_get(g_lang_resource,"MAIN_A", language));
		}
	...
	const char *part)
{
	...
	if (i < item_num) {
		...
		if (TYPE_MX == type) {
			len = sprintf(temp_line, "%s\tMX\t%s\n", temp_domain, temp_part);
		} else {
			len = sprintf(temp_line, "%s\tA\t%s\n", temp_domain, temp_part);
		}
		write(fd, temp_line, len);
		close(fd);
		list_file_free(pfile);
...
static void table_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/dns_adaptor.txt",
		...
		for (i=0; i<item_num; i++) {
			...
			if (0 == strcmp(pitem[i].type, "MX")) {
				len = sprintf(temp_line, "MX\t%s\t%s\n", temp_domain,
						pitem[i].part);
			} else {
				len = sprintf(temp_line, "A\t%s\t%s\n", temp_domain,
						pitem[i].part);
			}
			write(fd, temp_line, len);
		}
