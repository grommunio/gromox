...
'&port=' + port;}\n\
function ModifyItem(ip, port, memo) {\n\
opeform.ip.value=ip; opeform.port.value=port; opeform.memo.value=memo;}\n\
...
name=ip />&nbsp;:&nbsp;<INPUT type=text value=\"25\" size=2 tabindex=2 \n\
name=port /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text value=\"\" \n\
size=24 tabindex=3 name=memo /></TD></TR><TR><TD></TD><TD></TD><TD>\n\
...
"<TR class=ItemRow><TD>&nbsp;%s&nbsp;</TD><TD>&nbsp;%d&nbsp;</TD><TD>&nbsp;%s\
&nbsp;</TD><TD>&nbsp;<A href=\"javascript:DeleteItem('%s', %d)\">%s</A> | \n\
...
static void	list_ui_modify_list(const char *ip, int port, const char *memo);
...
int list_ui_run()
{
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			list_ui_modify_list(ip_addr, port, memo);
			list_ui_main_html(session);
			return 0;
		}
	} else {
...
static void list_ui_main_html(const char *session)
{
	...
	pfile = list_file_init(g_list_path, "%s:16%s:256%d");
	if (NULL == pfile) {
	...
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, pitem[i].ip, pitem[i].port,
				...
				pitem[i].ip, pitem[i].port, pitem[i].memo,
...
static void list_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/esmtp_auth.txt",
