...
function DeleteItem(boundary) {location.href='%s?session=%s&boundary=' + boundary;}\n\
function ModifyItem(boundary, memo) {\
opeform.boundary.value=boundary; opeform.memo.value=memo; opeform.memo.focus();}\n\
...
name=boundary /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text \
value=\"\" tabindex=2 name=memo /></TD></TR><TR><TD></TD><TD></TD><TD>\n\
<INPUT type=submit tabindex=3 value=\"%s\" onclick=\
\"if(opeform.boundary.value == 0) {return false;} else {return true;}\" \
/>\n\
...
static void	blacklist_ui_modify_list(const char *boundary, const char *memo);

static void blacklist_ui_remove_item(const char *boundary);
...
int blacklist_ui_run()
{
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			ptr1 += 8;
			ptr2 = search_string(search_buff, "&boundary=", len);
			if (NULL == ptr2) {
			...
			if (NULL == ptr2) {
				...
				memcpy(temp_boundary, ptr1, search_buff + len - ptr1 - 1);
				temp_boundary[search_buff + len - ptr1 - 1] = '\0';
				ltrim_string(temp_boundary);
				rtrim_string(temp_boundary);
				...
				blacklist_ui_remove_item(temp_boundary);
				blacklist_ui_main_html(session);
				return 0;
			}
			...
			memcpy(temp_boundary, ptr1, ptr2 - ptr1);
			temp_boundary[ptr2 - ptr1] = '\0';
			ltrim_string(temp_boundary);
			rtrim_string(temp_boundary);
			ptr1 = ptr2 + 6;
			if (search_buff + len - ptr1 - 1 > 256) {
			...
			blacklist_ui_modify_list(temp_boundary, memo);
			blacklist_ui_main_html(session);
			return 0;
		}
	} else {
...
static void blacklist_ui_main_html(const char *session)
{
	...
	for (i=0; i<item_num; i++) {
		...
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].boundary,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language),
				pitem[i].boundary, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].boundary, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		} else {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].boundary,
				pitem[i].memo, pitem[i].boundary,
...
static void	blacklist_ui_modify_list(const char *boundary, const char *memo)
{
	...
	item_num = list_file_get_item_num(pfile);
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(boundary, pitem[i].boundary)) {
			break;
		}
	}
	time(&cur_time);
...
static void blacklist_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/boundary_list.txt",
		...
		for (i=0; i<item_num; i++) {
			len = sprintf(temp_boundary, "%s\n", pitem[i].boundary);
			write(fd, temp_boundary, len);
		}
		close(fd);
	}
	closedir(dirp);
