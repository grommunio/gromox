...
</SCRIPT><BR><BR><I>%s</I><TABLE width=\"75%\"><TBODY>\n\
...
<TD><TEXTAREA name=local_ips %s cols=16 rows=6>"


#define HTML_MAIN_6	\
"</TEXTAREA></TD><TD><INPUT type=submit value=\"  %s  \" %s /></TD></FORM></TR>\n\
<TR class=ItemEven><TD colSpan=3>%s&nbsp;&nbsp;(%s)</TD></TR>\n\
<TR class=ItemOdd><FORM name=WebLST method=get action=%s>\n\
<TD><INPUT type=hidden name=\"session\" value=\"%s\"/></TD>\n\
<TD><INPUT type=text size=60 name=\"url_path\" value=\"%s\" %s/></TD>\n\
<TD><INPUT type=submit value=\"  %s  \" %s/></TD></FORM></TR>\n\
<TR class=ItemEven><TD colSpan=3>%s&nbsp;&nbsp;(%s)</TD></TR>\n\
<TR class=ItemOdd><FORM method=post action=%s enctype=\"multipart/form-data\">\n\
<TD><INPUT type=hidden name=\"session\" value=\"%s\" /></TD>\n\
<TD><INPUT type=file name=\"upload_list\" %s /></TD>\n\
<TD><INPUT type=submit value=\"  %s  \" %s /></TD></FORM></TR>\n\
<TR class=ItemOdd><TD></TD><TD colSpan=2><HR></TD></TR>\n\
<TR class=ItemOdd><FORM name=EditLST method=get action=%s>\n\
<TD><INPUT type=hidden name=\"session\" value=\"%s\" /></TD>\n\
<TD><INPUT type=text name=\"add_domain\" value=\"\" %s /></TD>\n\
...
#define HTML_TBITEM_DISABLED	\
"<TR class=ItemRow><TD>&nbsp;%s&nbsp;</TD><TD>&nbsp;</TD></TR>\n"
...
static void setup_ui_main_html(const char *session)
{
	...
	if (TRUE == b_sync && '1' == mask_string[INDEX_LOCAL_DOMAIN]) {
		str_disabled = PARAM_DISABLED;
		str_comment = lang_resource_get(g_lang_resource,"TIP_SYNCHRONIZATION", language);
	} else {
		str_disabled = PARAM_ENABLED;
		str_comment = "";
	}
	switch (temp_type) {
	case 0:
		printf(HTML_MAIN_5, url_buff, session, str_comment,
			lang_resource_get(g_lang_resource,"MAIN_CURRENT_TYPE", language),
			lang_resource_get(g_lang_resource,"MAIN_AUTODNS", language),
			lang_resource_get(g_lang_resource,"MAIN_AUTODNS", language), 
			lang_resource_get(g_lang_resource,"TIP_AUTODNS", language),
			url_buff, session, str_disabled);
		break;
	case 1:
		printf(HTML_MAIN_5, url_buff, session, str_comment,
			lang_resource_get(g_lang_resource,"MAIN_CURRENT_TYPE", language),
	...
	printf(HTML_MAIN_6, lang_resource_get(g_lang_resource,"SUBMIT_LABEL", language),
		str_disabled, lang_resource_get(g_lang_resource,"MAIN_WEBLIST", language),
		lang_resource_get(g_lang_resource,"TIP_WEBLIST", language), url_buff, session,
		str_value, str_disabled, lang_resource_get(g_lang_resource,"SUBMIT_LABEL", language),
		str_disabled, lang_resource_get(g_lang_resource,"MAIN_MANUALEDIT", language),
		lang_resource_get(g_lang_resource,"TIP_MANUALEDIT", language), url_buff, session,
		str_disabled, lang_resource_get(g_lang_resource,"UPLOAD_LABEL", language),
		str_disabled, url_buff, session, str_disabled,
		lang_resource_get(g_lang_resource,"ADD_LABEL", language), str_disabled);
	...
	if (2 == temp_type) {
		for (i=0; i<item_num; i++) {
			if (TRUE == b_sync && '1' == mask_string[INDEX_LOCAL_DOMAIN]) {
				printf(HTML_TBITEM_DISABLED, pitem + 256*i);
			} else {
