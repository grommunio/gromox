...
function DeleteItem(from) {location.href='%s?session=%s&from=' + from;}\n\
function ModifyItem(from, memo) {\
opeform.from.value=from; opeform.memo.value=memo; opeform.memo.focus();}\n\
...
name=from /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text \
...
static void	blacklist_ui_modify_list(const char *from, const char *memo);

static void blacklist_ui_remove_item(const char *from);
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			if (NULL == ptr2) {
				...
				memcpy(temp_from, ptr1, search_buff + len - ptr1 - 1);
				temp_from[search_buff + len - ptr1 - 1] = '\0';
				ltrim_string(temp_from);
				rtrim_string(temp_from);
				...
				blacklist_ui_remove_item(temp_from);
				blacklist_ui_main_html(session);
				return 0;
			}
			...
			memcpy(temp_from, ptr1, ptr2 - ptr1);
			temp_from[ptr2 - ptr1] = '\0';
			ltrim_string(temp_from);
			rtrim_string(temp_from);
			ptr1 = ptr2 + 6;
			if (search_buff + len - ptr1 - 1 > 256) {
			...
			blacklist_ui_modify_list(temp_from, memo);
			blacklist_ui_main_html(session);
			return 0;
		}
	} else {
...
static void blacklist_ui_main_html(const char *session)
{
	...
	for (i=0; i<item_num; i++) {
		...
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].from,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language),
				pitem[i].from, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].from, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		} else {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].from,
				pitem[i].memo, pitem[i].from,
...
static void	blacklist_ui_modify_list(const char *from, const char *memo)
{
	...
	item_num = list_file_get_item_num(pfile);
	for (i=0; i<item_num; i++) {
		if (0 == strcasecmp(from, pitem[i].from)) {
			break;
		}
	}
	time(&cur_time);
...
static void blacklist_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/forbidden_from.txt",
		...
		for (i=0; i<item_num; i++) {
			len = sprintf(temp_from, "%s\n", pitem[i].from);
			write(fd, temp_from, len);
		}
		close(fd);
	}
	closedir(dirp);
