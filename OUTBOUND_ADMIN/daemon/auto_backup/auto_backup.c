...
int auto_backup_run()
{
	...
	sprintf(src_file, "%s/auth_table.txt", g_data_path);
	sprintf(dst_file, "%s/data_files/auth_table.txt", g_backup_path);
	file_operation_copy_file(src_file, dst_file);
	
	sprintf(src_file, "%s/authorized_backup.txt", g_data_path);
	sprintf(dst_file, "%s/data_files/authorized_backup.txt", g_backup_path);
	file_operation_copy_file(src_file, dst_file);
	...
	len += sprintf(pbuff + node_stat.st_size + len,
			...
			"To: <%s>\r\n"
			"Subject: Anti-spam outbound gateway automatic backup of %s\r\n"
			"Date: %s\r\n"
			"Content-Transfer-Encoding: base64\r\n"
			"Content-Type: application/octet-stream;\r\n"
			"\tname=\"outbound_config.tgz\"\r\n"
			"Content-Disposition: attachment;\r\n"
			"\tfilename=\"outbound_config.tgz\r\n\r\n",
	...
	if (0 == strcasecmp(g_default_domain, pdomain)) {
		smtp_sender_send(TRUE, "auto-backup@system.mail", g_admin_mailbox,
			pbuff + node_stat.st_size, len);
	} else {
		sprintf(temp_buff, "auto-backup@%s", g_default_domain);
		smtp_sender_send(FALSE, temp_buff, g_admin_mailbox,
			pbuff + node_stat.st_size, len);
	}
	free(pbuff);
	remove(dst_file);
