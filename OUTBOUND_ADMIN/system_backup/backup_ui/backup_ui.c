...
static void backup_ui_restart_synchronizer();
...
AUTH_TABLE:
	sprintf(src_file, "%s/data_files/auth_table.txt", g_backup_path);
	sprintf(dst_file, "%s/auth_table.txt", g_data_path);
	...
	if (NULL == pfile) {
		system_log_info("[backup_ui]: fail to init auth hosts list file\n");
		goto BACKEND_TABLE;
	}
	...
	close(fd);
	list_file_free(pfile);
	file_operation_broadcast(temp_file, "data/smtp/esmtp_auth.txt");
...
CONSOLE_FLUSH:
	gateway_control_notify("esmtp_auth.svc hosts reload", NOTIFY_SMTP);
	...
	backup_ui_restart_synchronizer();
