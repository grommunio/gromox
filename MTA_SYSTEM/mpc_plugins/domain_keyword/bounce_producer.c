...
BOOL bounce_producer_refresh()
{
	...
	dirp = opendir(g_path);
	if (NULL == dirp) {
		printf("[domain_keyword]: fail to open directory %s\n", g_path);
		return FALSE;
	}
	...
	if (NULL == pdefault) {
		printf("[domain_keyword]: there's no \"en\" bounce mail "
			"templates\n");
...
static void bounce_producer_load_template(const char *temp_name, SINGLE_LIST *plist)
{
	...
	if (NULL == presource) {
		printf("[domain_keyword]: fail to allocate resource node memory\n");
		return;
	}
	...
	for (i=TAG_BEGIN+1; i<TAG_TOTAL_LEN; i++) {
		if (-1 == presource->format[i].position) {
			printf("[domain_keyword]: format error in %s, lack of "
...
void bounce_producer_make(MESSAGE_CONTEXT *pcontext, char *forward_to,
	char *language, char *keyword, char *url, MAIL *pmail)
{
	char *ptr;
	int i, len;
	BOOL b_first;
	...
	for (i=TAG_BEGIN+1; i<TAG_TOTAL_LEN; i++) {
		...
		case TAG_LENGTH:
			...
			if (-1 == mail_len) {
				printf("[domain_keyword]: fail to get mail length\n");
				mail_len = 0;
			}
	...
	if (NULL == pmime) {
		pthread_rwlock_unlock(&g_list_lock);
		printf("[domain_keyword]: fatal error, there's no mime "
			"in mime pool\n");
		return;
	}
	...
	pthread_rwlock_unlock(&g_list_lock);
	mime_set_content_type(pmime_child, tmp_buff);
	...
	if (FALSE == mime_write_content(pmime_child, original_ptr,
		...
		printf("[domain_keyword]: fatal error, fail to write content\n");
		return;
	}
	pmime_child = mail_add_child(pmail, pmime, MIME_ADD_LAST);
	if (NULL == pmime_child) {
