...
<HTML><HEAD>\
<STYLE TYPE=\"text/css\"><!--\r\n\
BODY {FONT-SIZE: 10pt;FONT-WEIGHT: bold;FONT-FAMILY: \r\n\
...
<SPAN class=AlarmTitle>Green Messenger Post Warning</SPAN>\r\n\
    ...
    int giveup_num)
{
	...
	g_giveup_num += giveup_num;
	if (0 != OK_num) {
		g_total_fail = 0;
	} else {
		g_total_fail += temp_fail + permanent_fail;
	}
	if (g_total_fail >= g_times && FALSE == g_turnoff_alarm) {
		need_alarm_one = TRUE;
	}
	g_fail_accumulating += temp_fail + permanent_fail;
	...
	if (TRUE == need_alarm_one || TRUE == need_alarm_two) {
		...
		if (0 == strcasecmp(pdomain, get_default_domain())) {
			strcpy(pcontext->pcontrol->from, "post-alarm@system.mail");
		} else {
			sprintf(pcontext->pcontrol->from, "post-alarm@%s",
				get_default_domain());
		}
		...
		if (TRUE == need_alarm_one) {
			...
			strcpy(tmp_buff + offset, ", don't be panic!\r\n"
						"<P></P><BR><P></P><BR><P></P><BR>Alarm time: ");
			offset += strlen(tmp_buff + offset);
		}
		...
		sprintf(tmp_buff,"Remote Postman Warning form %s", get_host_ID());
...
int net_failure_get_param(int param)
{
    ...
    case NET_FAILURE_PERMANENT:
		...
		pthread_mutex_unlock(&g_lock);
		return ret_val;
    case NET_FAILURE_GIVEUP:
		pthread_mutex_lock(&g_lock);
		ret_val = g_giveup_num;
		g_giveup_num = 0;
		pthread_mutex_unlock(&g_lock);
		return ret_val;
