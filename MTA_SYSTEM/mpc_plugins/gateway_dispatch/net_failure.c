    ...
    int nouser_num)
{
	...
	if (TRUE == need_alarm_one || TRUE == need_alarm_two) {
		...
		if (0 == strcasecmp(pdomain, get_default_domain())) {
			strcpy(pcontext->pcontrol->from, "dispatch-alarm@system.mail");
		} else {
			sprintf(pcontext->pcontrol->from, "dispatch-alarm@%s",
				get_default_domain());
		}
		...
		if (TRUE == need_alarm_one) {
			offset += sprintf(tmp_buff + offset, "  The gateway dispatch of %s "
		...
		sprintf(tmp_buff,"Gateway Dispatch Alarm form %s!!!", get_host_ID());
