...
int AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_tagging) {
			printf("[property_020]: fail to get \"check_tagging\" service\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[property_020]: error to open config file!!!\n");
			return FALSE;
		}
        ...
        if (FALSE == register_auditor(head_filter)) {
			printf("[property_020]: fail to register auditor function\n");
            return FALSE;
        }
        return TRUE;
    case PLUGIN_FREE:
