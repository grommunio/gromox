...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == ip_whitelist_query) {
			printf("[attach_filter]: failed to get service \"ip_whitelist_query\"\n");
			return FALSE;
		}
		check_tagging = (CHECK_TAGGING)query_service("check_tagging");
		if (NULL == check_tagging) {
			printf("[attach_filter]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[attach_filter]: config_file_init %s: %s\n", temp_path, strerror(errno));
			return FALSE;
		}
        ...
        if (FALSE == register_filter(NULL , attach_name_filter)) {
			free(g_context_list);
			g_context_list = NULL;
			printf("[attach_filter]: failed to register filter function\n");
            return FALSE;
        }
        return TRUE;
    case PLUGIN_FREE:
