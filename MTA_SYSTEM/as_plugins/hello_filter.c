...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_retrying) {
			printf("[hello_filter]: failed to get service \"check_retrying\"\n");
			return FALSE;
		}
		check_tagging = (CHECK_TAGGING)query_service("check_tagging");
		if (NULL == check_tagging) {
			printf("[hello_filter]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
		ip_whitelist_query = (WHITELIST_QUERY)query_service(
				            "ip_whitelist_query");
		if (NULL == ip_whitelist_query) {
			printf("[hello_filter]: failed to get service \"ip_whitelist_query\"\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[hello_filter]: config_file_init %s: %s\n", temp_path, strerror(errno));
			return FALSE;
		}
        ...
        if (FALSE == register_judge(envelop_judge)) {
			printf("[hello_filter]: failed to register judge function\n");
            return FALSE;
        }
        return TRUE;
    case PLUGIN_FREE:
