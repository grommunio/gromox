...
int AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == domain_whitelist_query) {
			printf("[address_checker]: fail to get \"domain_whitelist_query\" "
					"service\n");
			return FALSE;
		}
		...
		if (NULL == ip_whitelist_query) {
			printf("[address_checker]: fail to get \"ip_whitelist_query\" "
					"service\n");
			return FALSE;
		}
		check_tagging = (CHECK_TAGGING)query_service("check_tagging");
		if (NULL == check_tagging) {
			printf("[address_checker]: fail to get \"check_tagging\" service\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[address_checker]: error to open config file!!!\n");
			return FALSE;
		}
        ...
        if (FALSE == register_judge(envelop_judge)) {
            printf("[address_checker]: fail to register judge function!!!\n");
            return FALSE;
        }
        return TRUE;
    case PLUGIN_FREE:
