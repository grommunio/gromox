...
#define SPAM_STATISTIC_PROPERTY_021		58
...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_tagging) {
			printf("[property_021]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
		...
		if (NULL == str_value) {
			strcpy(g_return_reason, "000058 you are now sending spam mail!");
		} else {
			strcpy(g_return_reason, str_value);
		}
		printf("[property_021]: return string is \"%s\"\n", g_return_reason);
	...
	CONNECTION *pconnection, char *reason, int length)
{
	...
	if (0 != strcasecmp(buff, "text/plain; charset=iso-2022-jp")) {
		return MESSAGE_ACCEPT;
	}
			...
			if (0 == strncasecmp("Error-to", buff, 8)) {
				...
				if (TRUE == check_tagging(pmail->penvelop->from, &pmail->penvelop->f_rcpt_to)) {
					...
					if (NULL != spam_statistic) {
						spam_statistic(SPAM_STATISTIC_PROPERTY_021);
					}
