...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == subject_audit) {
			printf("[subject_auditor]: failed to get service \"subject_audit\"\n");
			return FALSE;
		}
		ip_whitelist_query = (WHITELIST_QUERY)query_service(
                             "ip_whitelist_query");
        if (NULL == ip_whitelist_query) {
            printf("[subject_auditor]: failed to get service \"ip_whitelist_query\"\n");
            return FALSE;
        }
        domain_whitelist_query = (WHITELIST_QUERY)query_service(
                                 "domain_whitelist_query");
        if (NULL == domain_whitelist_query) {
            printf("[subject_auditor]: failed to get service \"domain_whitelist_query\"\n");
            return FALSE;
        }
		check_tagging = (CHECK_TAGGING)query_service("check_tagging");
		if (NULL == check_tagging) {
			printf("[subject_auditor]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
        ...
        if (FALSE == register_auditor(subject_auditor)) {
			printf("[subject_auditor]: failed to register auditor function\n");
            return FALSE;
        }
        return TRUE;
    case PLUGIN_FREE:
