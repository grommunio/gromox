...
#define SPAM_STATISTIC_PROPERTY_024		61
...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_tagging) {
			printf("[property_024]: fail to get \"check_tagging\" service\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[property_024]: error to open config file!!!\n");
			return FALSE;
		}
		str_value = config_file_get_value(pconfig_file, "RETURN_STRING");
		if (NULL == str_value) {
			strcpy(g_return_reason, "000061 you are now sending spam mail!");
		} else {
			strcpy(g_return_reason, str_value);
		}
		printf("[property_024]: return string is %s\n", g_return_reason);
	...
	CONNECTION *pconnection, char *reason, int length)
{
	...
	if (NULL == search_string(buff, "boundary=\"----=Part.", out_len)) {
		return MESSAGE_ACCEPT;
	}
			...
			if (0 == strncasecmp(buff, "Content-Transfer-Encoding", 25)) {
				if (TRUE == check_tagging(pmail->penvelop->from, &pmail->penvelop->f_rcpt_to)) {
					...
					if (NULL != spam_statistic) {
						spam_statistic(SPAM_STATISTIC_PROPERTY_024);
					}
