...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_retrying) {
			printf("[property_013]: fail to get \"check_retrying\" service\n");
			return FALSE;
		}
		check_tagging = (CHECK_TAGGING)query_service("check_tagging");
		if (NULL == check_tagging) {
			printf("[property_013]: fail to get \"check_tagging\" service\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[property_013]: error to open config file!!!\n");
			return FALSE;
		}
