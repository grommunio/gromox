...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_tagging) {
			printf("[property_019]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[property_019]: config_file_init %s: %s\n", temp_path, strerror(errno));
			return FALSE;
		}
        ...
        if (FALSE == register_auditor(head_filter)) {
			printf("[property_019]: failed to register auditor function\n");
            return FALSE;
        }
        return TRUE;
    case PLUGIN_FREE:
