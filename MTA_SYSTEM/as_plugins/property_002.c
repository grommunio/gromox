...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_tagging) {
			printf("[property_002]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
		check_retrying = (CHECK_RETRYING)query_service("check_retrying");
		if (NULL == check_retrying) {
			printf("[property_002]: failed to get service \"check_retrying\"\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[property_002]: config_file_init %s: %s\n", temp_path, strerror(errno));
			return FALSE;
		}
