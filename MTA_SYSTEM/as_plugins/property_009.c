...
BOOL AS_LibMain(int reason, void **ppdata)
{
    ...
    case PLUGIN_INIT:
		...
		if (NULL == check_tagging) {
			printf("[property_009]: failed to get service \"check_tagging\"\n");
			return FALSE;
		}
		domain_whitelist_query = (WHITELIST_QUERY)
			query_service("domain_whitelist_query");
		if (NULL == domain_whitelist_query) {
			printf("[property_009]: failed to get service \"domain_whitelist_query\"\n");
			return FALSE;
		}
		...
		if (NULL == pconfig_file) {
			printf("[property_009]: config_file_init %s: %s\n", temp_path, strerror(errno));
			return FALSE;
		}
