...
static void mail_enum_depth(SIMPLE_TREE_NODE *pnode, void *param);

static void mail_enum_delete(SIMPLE_TREE_NODE *pnode);

static SIMPLE_TREE_NODE* mail_enum_copy(
	SIMPLE_TREE_NODE *pnode, void *param);
...
/*
 *	retrieve buffer into mail object, and memory will be 
 *	managed by mail self
 *	@param
 *		pmail [in]			indicate the mail object
...
BOOL mail_retrieve_ex(MAIL *pmail, char *in_buff, size_t length)
{
	...
	if (FALSE == mail_retrieve(pmail, pbuff, length)) {
		free(pbuff);
		return FALSE;
	} else {
		pmail->buffer = pbuff;
...
/*
 *	get the depth of mail object
 *	@param
 *		pmail [in]			indicate the mail object
 *	@return
 *		depth of mime(s), counted from 0
 *		"negative" means none
 */
...
#ifdef _DEBUG_UMTA
	if (NULL == pmail) {
		debug_info("[mail]: NULL pointer in mail_get_depth");
		return -1;
	}
#endif
	ret_val = -1;
	simple_tree_enum_from_node(simple_tree_get_root(&pmail->tree),
        mail_enum_depth, &ret_val);
	return ret_val;
}
...
#ifdef _DEBUG_UMTA
    if (NULL == pnode || NULL == param) {
        debug_info("[mail]: NULL pointer in mail_enum_depth");
        return;
    }
#endif
...
/*
 *	get the mime from mail tree, first along the horizonal line
 *	@param
 *		pmail [in]	indicate the mail object
 *		pmime_base	base mime where we count from, NULL means root
 *		x			x position, counted from 0
 *		y           y position, counted from 0
 *	@return
 *		pointer to mime object
 */
...
#ifdef _DEBUG_UMTA
	if (NULL == pmail) {
		debug_info("[mail]: NULL pointer in mail_get_mime_horizontal");
		return NULL;
	}
#endif
	if (NULL == pmime_base) {
		pnode = simple_tree_get_node_horizontal(&pmail->tree,
			simple_tree_get_root(&pmail->tree), x, y);
	} else {
		pnode = simple_tree_get_node_horizontal(&pmail->tree,
			&pmime_base->node, x, y);
	}
...
/*
 *	get the mime from mail tree, first along the vertical line
 *	@param
 *		pmail [in]	indicate the mail object
 *		pmime_base	base mime where we count from, NULL means root
 *		x			x position, counted from 0
...
#ifdef _DEBUG_UMTA
    if (NULL == pmail) {
        debug_info("[mail]: NULL pointer in mail_get_mime_vertical");
        return NULL;
    }
#endif
	if (NULL == pmime_base) {
        pnode = simple_tree_get_node_vertical(&pmail->tree,
            simple_tree_get_root(&pmail->tree), x, y);
    } else {
    	pnode = simple_tree_get_node_vertical(&pmail->tree,
			&pmime_base->node, x, y);
...
/*
 *	delete the mime and all its descendant mimes
 *	@param
 *		pmail [in]		indicate the mail object
 *		pmime [in]		the mime where begin from
 */
void mail_delete_mime(MAIL *pmail, MIME *pmime)
{
#ifdef _DEBUG_UMTA
	if (NULL == pmail || NULL == pmime) {
		debug_info("[mail]: NULL pointer in mail_delete_mime");
		return;
	}
#endif
	simple_tree_destroy_node(&pmail->tree, &pmime->node, mail_enum_delete);
}
...
/*
 *  delete the mime's descendant mimes, not including mime itself
 *  @param
 *      pmail [in]      indicate the mail object
 *      pmime [in]      the mime where begin from
 */
...
#ifdef _DEBUG_UMTA
    if (NULL == pmail || NULL == pmime) {
        debug_info("[mail]: NULL pointer in mail_empty_children");
        return;
    }
#endif
	simple_tree_empty_children(&pmail->tree, &pmime->node, mail_enum_delete);
}
 ...
 *      pmail_src [in]			indicate the source mail object
 *      pmime_src [in]      	source mime
 *		opt						MIME_ADD_FIRST
 *								MIME_ADD_LAST
 *  @return
 *      TRUE                	OK
	...
	MAIL *pmail_src, MIME *pmime_src, int opt)
{
    ...
    if (NULL == pmail_dst || NULL == pmime_dst ||
        ...
        debug_info("[mail]: NULL pointer in mail_move_mime_to_child");
        return FALSE;
    }
#endif
	if (MULTIPLE_MIME != pmime_dst->mime_type) {
		return FALSE;
	}
	return simple_tree_move_node_to_child(&pmail_dst->tree,
 ...
 *      pmail_dst [in]          indicate the destination mail object
 *      pmime_dst [in]      	destination mime where we begin from
 *		pmail_dst [in]			indicate the source mail object
 *      pmime_src [in]      	source mime
 *		opt						MIME_INSERT_AFTER
 *								MIME_INSERT_BEFORE
 *  @return
 *      TRUE                	OK
 *      FALSE               	fail
	...
	MAIL *pmail_src, MIME *pmime_src, int opt)
{
    ...
    if (NULL == pmail_dst || NULL == pmime_dst ||
		NULL == pmail_src || NULL == pmime_src) {
        debug_info("[mail]: NULL pointer in mail_move_children_to_child");
        return FALSE;
    }
#endif
    if (MULTIPLE_MIME != pmime_dst->mime_type) {
    ...
    return simple_tree_move_children_to_child(&pmail_dst->tree,
        &pmime_dst->node, &pmail_src->tree, &pmime_src->node, opt);
}
	...
	MIME *pmime_src, int opt)
{
    ...
    if (NULL == pmail_dst || NULL == pmime_dst ||  NULL == pmime_src) {
        debug_info("[mail]: NULL pointer in mail_copy_mime_to_child");
        return FALSE;
    }
#endif
	if (MULTIPLE_MIME != pmime_dst->mime_type) {
	...
	return simple_tree_copy_node_to_child(&pmail_dst->tree,
		&pmime_dst->node, &pmime_src->node, opt, mail_enum_copy,
		pmail_dst->pmime_pool);
}

static SIMPLE_TREE_NODE* mail_enum_copy(SIMPLE_TREE_NODE *pnode, void *param)
{
	MIME *pmime;

#ifdef _DEBUG_UMTA
    if (NULL == pnode) {
        debug_info("[mail]: NULL pointer in mail_enum_copy");
        return NULL;
    }
#endif
	pmime = mime_pool_get((MIME_POOL*)param);
	if (NULL == pmime) {
		return NULL;
	}
	mime_copy((MIME*)(pnode->pdata), pmime);
	return &pmime->node;
}
	...
	MIME *pmime_src, int opt)
{
#ifdef _DEBUG_UMTA
    if (NULL == pmail_dst || NULL == pmime_dst || NULL == pmime_src) {
        debug_info("[mail]: NULL pointer in mail_copy_children_to_child");
        return FALSE;
    }
#endif
    if (MULTIPLE_MIME != pmime_dst->mime_type) {
    ...
    return simple_tree_copy_children_to_child(&pmail_dst->tree,
        &pmime_dst->node, &pmime_src->node, opt, mail_enum_copy,
		pmail_dst->pmime_pool);
}
