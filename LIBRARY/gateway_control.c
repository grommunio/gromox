...
static BOOL gateway_control_exec(const char *ip, int port, const char *command)
{
	BOOL result;
	int sockd, cmd_len;
	int read_len, offset;
...
BOOL gateway_control_activate(const char *command, int control_mask)
{
	...
	for (pnode=single_list_get_head(&g_console_list); NULL!=pnode;
		...
		pconsole = (CONSOLE_PORT*)pnode->pdata;
		if (NOTIFY_SMTP&control_mask) {
			if (TRUE == gateway_control_exec(pconsole->smtp_ip,
				pconsole->smtp_port, command)) {
				return TRUE;
			}
		}
		if (NOTIFY_DELIVERY&control_mask) {
			if (TRUE == gateway_control_exec(pconsole->delivery_ip,
				pconsole->delivery_port, command)) {
				return TRUE;
			}
		}
