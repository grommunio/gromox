...
void remove_digest(char *src, const char *tag)
{
	...
	ptr2 = ptr1 + len;
	ptr2 = memchr(ptr2, ':', temp_len - (ptr2 - src));
	if (NULL == ptr2) {
		return;
	}
	ptr2 ++;
...
long atoitvl(const char *string)
{
	...
	for (i=0; i<length; i++) {
		tmp = temp_buff[i];
		if ((tmp >= '0' && tmp <= '9') || 'd' == tmp || 'D' == tmp ||
			'h' == tmp || 'H' == tmp || 'm' == tmp || 'M' == tmp ||
			's' == tmp || 'S' == tmp || ' ' == tmp || ',' == tmp ||
			'\t' == tmp) {	
			switch (tmp) {
			case 'd':
			case 'D':
				...
				if (0 == strncasecmp(temp_buff + i, "day", 3)) {
					temp_buff[i] = '\0';
					days = atol(pbegin);
					if ('s' == temp_buff[i + 3] || 'S' == temp_buff[i + 3]) {
						i += 3;
					} else {
						i += 2;
					}
					pbegin = temp_buff + i + 1;
					break;
				} else if (' ' == temp_buff[i + 1] || ',' == temp_buff[i + 1] ||
					'\t' == temp_buff[i + 1] || '\0' == temp_buff[i + i] ||
					('0' <= temp_buff[i + 1] && '9' >= temp_buff[i + 1])) {
					temp_buff[i] = '\0';
					days = atol(pbegin);
					pbegin = temp_buff + i + 1;
					break;
			...
			case 'H':
				...
				if (0 == strncasecmp(temp_buff + i, "hour", 4)) {
					temp_buff[i] = '\0';
					hours = atol(pbegin);
					if ('s' == temp_buff[i + 4] || 'S' == temp_buff[i + 4]) {
						i += 4;
					} else {
						i += 3;
					}
			...
			case 'M':
				...
				if (0 == strncasecmp(temp_buff + i, "minute", 6)) {
					temp_buff[i] = '\0';
					minutes = atol(pbegin);
					if ('s' == temp_buff[i + 6] || 'S' == temp_buff[i + 6]) {
						i += 6;
					} else {
						i += 5;
					}
			...
			case 'S':
				...
				if (0 == strncasecmp(temp_buff + i, "second", 6)) {
					temp_buff[i] = '\0';
					seconds = atol(pbegin);
					if ('s' == temp_buff[i + 6] || 'S' == temp_buff[i + 6]) {
						i += 6;
	...
	if (-1 == days && -1 == hours && -1 == minutes && -1 == seconds) {
		return atol(string);
	} else {
		ret_val = 0;
		if (-1 != days) {
...
#define WILDP '%'  /* match characters in current level (only for imap) */
	...
	const char *mask, BOOL icase)
{
  ...
  while (data >= na) {
	...
	case WILDS:				   /* Matches anything */
	  do
		mask--;					   /* Zap redundant wilds */
	  while ((mask >= ma) && ((*mask == WILDS) || (*mask == WILDP)));
	  type = WILDS;
	  lsm = mask;
	  lsn = data;
	  match += sofar;
