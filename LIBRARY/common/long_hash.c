...
LONG_HASH_TABLE* long_hash_init(size_t max_items, size_t item_size, PLONG_HASH_FUNC fun)
{
	...
	PLONG_HASH_TABLE  table = NULL;
	size_t	i = 0;

	if (max_items <= 0 || item_size <= 0) {
		debug_info("[long_hash]: parameter is invalid");
		return NULL;
	}
	table = (PLONG_HASH_TABLE)malloc(sizeof(LONG_HASH_TABLE));
	if (NULL == table) {
		debug_info("[long_hash]: can not alloc hash table");
		return NULL;
	}
	...
	if (NULL == p_map) {
		debug_info("[long_hash]: can not alloc hash map");
		free(table);
		return NULL;
	}
	...
	table->buf_pool = lib_buffer_init(sizeof(LONG_HASH_ITEM) + item_size,
						max_items, FALSE);
	if (NULL == table->buf_pool) {
		debug_info("[long_hash]: long_hash_init, lib_buffer_init fail");
		free(table);
		free(p_map);
		return NULL;
	...
	if (NULL == fun) {
		table->hash_func = default_long_hash_function;
	} else {
		table->hash_func = fun;
	}
...
int long_hash_add(LONG_HASH_TABLE* ptbl, long key, void *value)
{
	...
	void* list_node = NULL;
	LONG_HASH_ITEM* item = NULL;
	size_t index = -1;
	
	if (NULL == ptbl || NULL == value) {
		debug_info("[long_hash]: long_hash_add, invalid parameter");
		return -1;
	}

	if (ptbl->item_num >= ptbl->capacity) {
		debug_info("[long_hash]: long_hash_add, the hash table is full");
		return -2;
	}
	...
	if (NULL == list_node) {
		debug_info("[long_hash]: long_hash_add, lib_buffer_get fail");
		return -3;
	}

	index = ptbl->hash_func(key) % (ptbl->entry_num);

	item  = (LONG_HASH_ITEM *)list_node;
	item->hash_key	= key;
	item->map_index = index;
	item->list_node.pdata = item;
	item->iter_node.pdata = item;

	memcpy((char*)list_node + sizeof(LONG_HASH_ITEM), value, ptbl->data_size);
	...
	if (NULL == dlist->phead) {
		...
		/* 
		 ...
		 the same key twice.
		 */
		if (key == ((LONG_HASH_ITEM*)dlist->phead->pdata)->hash_key) {
			debug_info("[long_hash]: long_hash_add, the key already exist");
		...
		{
			if (key == ((LONG_HASH_ITEM*)next->pdata)->hash_key) {
				debug_info("[long_hash]: long_hash_add, the key already exist");
				lib_buffer_put(ptbl->buf_pool, list_node);
				return -4;
			}
...
void* long_hash_query(LONG_HASH_TABLE* ptbl, long key)
{
...
#ifdef _DEBUG_UMTA
	if (NULL == ptbl) {
		debug_info("[long_hash]: long_hash_query, invalid param");
		return NULL;
	}
#endif
	index = ptbl->hash_func(key) % (ptbl->entry_num);
	...
	if (key == ((LONG_HASH_ITEM*)next->pdata)->hash_key) {
		pdata = (char*)next->pdata + sizeof(LONG_HASH_ITEM);
		return pdata;
	}
	next = next->pnext;
...
#ifdef _DEBUG_UMTA
	if (NULL == ptbl) {
		debug_info("[long_hash]: long_hash_remove, invalid param");
		return -1;
	}
#endif
	index = ptbl->hash_func(key) % (ptbl->entry_num);
...
DONE:
	...
	double_list_remove(&(ptbl->iter_list), 
				&(((LONG_HASH_ITEM*)next->pdata)->iter_node));
...
{
	PLONG_HASH_ITER iter = NULL;

#ifdef _DEBUG_UMTA
	if (NULL == ptbl) {
		debug_info("[long_hash]: long_hash_iter_init, invalid parameter");
		return NULL;
	}
#endif
	
	if (NULL == (iter = malloc(sizeof(LONG_HASH_ITER)))) {
		debug_info("[long_hash]: can not alloc hash iter");
		return NULL;
	}
 ...
 *			
 *		[code]:
 *		for (piter=long_hash_iter_begin(piter); !long_hash_iter_done(piter)
 *			long_hash_iter_forward(piter)) {
 *			value = long_hash_iter_get_value(piter, NULL);
 *		}
 */
...
#ifdef _DEBUG_UMTA
	if (NULL == piter) {
		debug_info("[long_hash]: long_hash_iter_get_value, invalid param");
		return NULL;
	}
#endif
	pvalue = (char *)piter->cur_node->pdata + sizeof(LONG_HASH_ITEM);
	
	if (NULL != key) {
		*key = ((LONG_HASH_ITEM*)piter->cur_node->pdata)->hash_key;
	}
	return pvalue;
}
...
#ifdef _DEBUG_UMTA
	if (NULL == piter) {
		debug_info("[long_hash]: long_hash_iter_forward, invalid param");
		return -1;
	}
#endif
...
int long_hash_iter_remove(LONG_HASH_ITER *piter)
{
	...
	LONG_HASH_ITEM* list_item  = NULL;

#ifdef _DEBUG_UMTA
	if (NULL == piter) {
		debug_info("[long_hash]: long_hash_iter_remove, invalid param");
		return -1;
	}
#endif
	...
	if (piter->ptable->item_num < 1) {
		debug_info("[long_hash]: the hash table is empty");
		return -2;
	}
	if (NULL == piter->cur_node) {
		debug_info("[long_hash]: the removed item does not exist");
		return -3;
	}
	node = piter->cur_node;
	...
	list_item = (LONG_HASH_ITEM*)node->pdata;
...
//////////////////////////////////////////////////////////////////
long long_hash_iter_backward(IP_HASH_ITER *piter)
{
	if (NULL == piter) {
...
static size_t default_long_hash_function(long key)
{

#ifdef __x86_64__
	key = (key*11400714819323198485UL) >> 28;
#else
	key = (key*2654435769UL) >> 28;
