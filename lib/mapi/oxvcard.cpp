...
BOOL oxvcard_export(MESSAGE_CONTENT *pmsg, VCARD *pvcard, GET_PROPIDS get_propids)
{
	...
	if (!oxvcard_get_propids(&propids, get_propids))
		return FALSE;
	pvcard->clear();
	auto ret = pvcard->append_line2("PROFILE", "VCARD");
	if (ret != ecSuccess)
		return false;
	ret = pvcard->append_line2("VERSION", "4.0");
	if (ret != ecSuccess)
		return false;
	ret = pvcard->append_line2("MAILER", "gromox-oxvcard");
	if (ret != ecSuccess)
		return false;
	ret = pvcard->append_line2("PRODID", "gromox-oxvcard");
	if (ret != ecSuccess)
		return false;
	...
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("FN", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	if (pvline == nullptr)
		return false;
	ret = pvcard->append_line2(pvline);
	if (ret != ecSuccess)
		return false;
	for (size_t i = 0; i < 5; ++i) {
		...
		if (pvalue == nullptr)
			continue;
		ret = pvline->append_value(pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("NICKNAME", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	for (size_t i = 0; i < 3; ++i) {
		...
		if (pvparam == nullptr)
			return false;
		ret = pvline->append_param(pvparam);
		if (ret != ecSuccess)
			return false;
		ret = pvparam->append_paramval("INTERNET");
		if (ret != ecSuccess)
			return false;
		if (i == 0) {
			ret = pvparam->append_paramval("PREF");
			if (ret != ecSuccess)
				return false;
		}
		ret = pvline->append_value(pvalue);
	...
	if (pvalue != nullptr && *reinterpret_cast<const uint8_t *>(pvalue) != 0 &&
		...
		for (size_t i = 0; i < pmsg->children.pattachments->count; ++i) {
			...
			if (ret != ecSuccess)
				return false;
			ret = pvline->append_param("TYPE", photo_type);
			if (ret != ecSuccess)
				return false;
			ret = pvline->append_param("ENCODING", "B");
			if (ret != ecSuccess)
				return false;
			if (encode64(bv->pb, bv->cb, tmp_buff, VCARD_MAX_BUFFER_LEN - 1, &out_len) != 0)
				return false;
			tmp_buff[out_len] = '\0';
			ret = pvline->append_value(tmp_buff);
			if (ret != ecSuccess)
				return false;
			break;
		}
	...
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("NOTE", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	ret = pvcard->append_line2("CLASS", pvalue);
	if (ret != ecSuccess)
		return false;
	...
	if (ret != ecSuccess)
		return false;
	ret = pvline->append_param("TYPE", "WORK");
	if (ret != ecSuccess)
		return false;
	...
	if (ret != ecSuccess)
		return false;
	ret = pvline->append_param("TYPE", "HOME");
	if (ret != ecSuccess)
		return false;
	for (size_t i = 0; i < 6; ++i) {
	...
	if (ret != ecSuccess)
		return false;
	ret = pvline->append_param("TYPE", "POSTAL");
	if (ret != ecSuccess)
		return false;
	for (size_t i = 0; i < 6; ++i) {
	...
	for (size_t i = 0; i < 10; ++i) {
		...
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_param("TYPE", tel_types[i]);
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_value(pvalue);
		if (ret != ecSuccess)
	...
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("ROLE", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	pvalue = pmsg->proplist.get<char>(proptag);
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("X-MS-OL-DESIGN", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	if (NULL != pvalue) {
		for (size_t i = 0; i < saval->count; ++i) {
			ret = pvcard->append_line2("X-MS-CHILD", saval->ppstr[i]);
			if (ret != ecSuccess)
				return false;
		}
	}
	...
	for (size_t i = 0; i < 4; ++i) {
		...
		if (pvalue == nullptr)
			continue;
		ret = pvcard->append_line2("X-MS-TEXT", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	for (size_t i = 0; i < 5; ++i) {
		...
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_param("TYPE", ms_tel_types[i]);
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_value(pvalue);
		if (ret != ecSuccess)
	...
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("UID", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	pvalue = pmsg->proplist.get<char>(proptag);
	if (pvalue != nullptr) {
		ret = pvcard->append_line2("FBURL", pvalue);
		if (ret != ecSuccess)
			return false;
	}
	...
	ret = pvcard->append_line2("TITLE", pvalue);
	if (ret != ecSuccess)
		return false;
	...
	pvalue = pmsg->proplist.get<char>(proptag);
	ret = pvcard->append_line2("X-MS-IMADDRESS", pvalue);
	if (ret != ecSuccess)
		return false;
	...
	if (NULL != pvalue) {
		...
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_param("VALUE", "DATE");
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_value(tmp_buff);
		if (ret != ecSuccess)
	...
	if (NULL != pvalue) {
		...
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_param("VALUE", "DATE-TIME");
		if (ret != ecSuccess)
			return false;
		ret = pvline->append_value(tmp_buff);
		if (ret != ecSuccess)
