    ...
    const PROPID_ARRAY &dst)
{
	 ...
	 * get_named_propids stop creating names and report id 0 for the ones it
	 * did not have — while still succeeding overall. Keeping the source id
	 * in that case would not preserve the property, it would name an
	 * unrelated one in the target store, so such properties have to go.
	 *
	 * Removal happens before rewriting because erase() matches on the whole
	 * proptag, and a tag produced by rewriting can coincide with one still
	 * queued for removal.
	 */
	...
	for (auto &pv : props) {
		...
		pv.proptag = PROP_TAG(PROP_TYPE(oldtag), newid);
	}
	return drop.size();
}
    ...
    std::unique_ptr<MESSAGE_CONTENT, mc_delete> &out)
{
	...
	if (err != ecSuccess)
		return err;
	/*
	 * Only the top-level proplist, unlike the named-property pass above
	 * which has to descend: every tag in the three lists is a property of a
	 * message as an object in a store. An embedded message is stored inside
	 * its attachment rather than as an addressable message of its own, so
	 * its copies of these carry no meaning to strip.
	 */
