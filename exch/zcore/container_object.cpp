	...
	const TPROPVAL_ARRAY *ppropvals, const RESTRICTION *pfilter)
{
	...
	case RES_OR:
		...
		return FALSE;
	case RES_NOT:
		return !container_object_match_contact_message(ppropvals, &pfilter->xnot->res);
	case RES_CONTENT: {
	...
	case RES_EXIST:
		return ppropvals->has(pfilter->exist->proptag);
	default:
		return FALSE;
	}
	return false;
...
ec_error_t container_object::load_user_table(const RESTRICTION *prestriction) try
{
	auto pcontainer = this;
	BINARY tmp_bin;
	uint32_t tmp_int;
	...
	if (CONTAINER_TYPE_ABTREE == pcontainer->type) {
		if (NULL == prestriction ||
		    pcontainer->contents.pminid_array != nullptr)
			return ecSuccess;
	...
	for (size_t i = 0; i < tmp_set.count; ++i) {
		for (unsigned int j = 0; j < 3; ++j) {
			...
			auto err = ppropvals->set(PR_SMTP_ADDRESS, username.c_str());
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_ACCOUNT, username.c_str());
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_ADDRTYPE, addrtype);
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_EMAIL_ADDRESS, username.c_str());
			if (err != ecSuccess)
				return err;
			if (NULL != pdisplayname) {
				err = ppropvals->set(PR_DISPLAY_NAME, pdisplayname);
				if (err != ecSuccess)
					return err;
				err = ppropvals->set(PR_TRANSMITABLE_DISPLAY_NAME, pdisplayname);
				if (err != ecSuccess)
					return err;
				err = ppropvals->set(PR_EMS_AB_DISPLAY_NAME_PRINTABLE, pdisplayname);
				if (err != ecSuccess)
					return err;
			}
			for (size_t k = 0; k < std::size(tmp_proptags); ++k) {
				...
				if (newval == nullptr)
					continue;
				err = ppropvals->set(tag, newval);
				if (err != ecSuccess)
					return err;
			}
			err = ppropvals->set(PR_PARENT_ENTRYID, pparent_entryid);
			if (err != ecSuccess)
				return err;
			...
			if (pvalue == nullptr)
				return ecError;
			err = ppropvals->set(PR_ENTRYID, pvalue);
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_RECORD_KEY, pvalue);
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_TEMPLATEID, pvalue);
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_ORIGINAL_ENTRYID, pvalue);
			if (err != ecSuccess)
				return err;
			tmp_bin.cb = sizeof(muidZCSAB);
			tmp_bin.pv = deconst(&muidZCSAB);
			err = ppropvals->set(PR_AB_PROVIDER_ID, &tmp_bin);
			if (err != ecSuccess)
				return err;
			tmp_int = static_cast<uint32_t>(mot);
			err = ppropvals->set(PR_OBJECT_TYPE, &tmp_int);
			if (err != ecSuccess)
				return err;
			tmp_int = static_cast<uint32_t>(dt);
			err = ppropvals->set(PR_DISPLAY_TYPE, &tmp_int);
			if (err != ecSuccess)
				return err;
			err = ppropvals->set(PR_DISPLAY_TYPE_EX, &tmp_int);
			if (err != ecSuccess)
				return err;
			if (prestriction != nullptr &&
			    !container_object_match_contact_message(ppropvals.get(), prestriction))
				continue;
			err = pcontainer->contents.prow_set->append_move(std::move(ppropvals));
			if (err != ecSuccess)
				return err;
...
ec_error_t container_object_fetch_special_property(uint8_t special_type,
    proptag_t proptag, void **ppvalue)
{
	...
	case PR_ENTRYID: {
		...
		if (bv->pv == nullptr)
			return ecServerOOM;
		if (!ext_push.init(static_cast<BINARY *>(pvalue)->pb, 128, 0) ||
...
static ec_error_t container_object_fetch_special_properties(uint8_t special_type,
    proptag_cspan tags, TPROPVAL_ARRAY *ppropvals)
{
	...
	for (const auto tag : tags) {
		void *pvalue = nullptr;
		auto err = container_object_fetch_special_property(special_type,
		           tag, &pvalue);
...
static ec_error_t
container_object_fetch_folder_properties(const TPROPVAL_ARRAY *ppropvals,
	...
	for (const auto tag : tags) {
		...
		case PR_PARENT_ENTRYID: {
			...
			if (tag != PR_PARENT_ENTRYID) {
				...
				auto err = container_object_fetch_special_property(
				           ab_tree::minid::SC_PROVIDER,
				           PR_ENTRYID, &pvalue);
				if (err != ecSuccess)
					return err;
			} else {
...
ec_error_t container_object::get_props(proptag_cspan pproptags,
    TPROPVAL_ARRAY *ppropvals)
{
	...
	if (pcontainer->type != CONTAINER_TYPE_ABTREE) {
		...
		if (!exmdb_client->get_folder_properties(pinfo->get_maildir(),
			...
			return ecRpcFailed;
		return container_object_fetch_folder_properties(&tmp_propvals,
		       pproptags, ppropvals);
	}
...
ec_error_t container_object::get_container_table_num(bool b_depth, uint32_t *pnum)
{
	auto pcontainer = this;
	TARRAY_SET tmp_set;
	PROPTAG_ARRAY proptags;
...
static ec_error_t container_object_query_folder_hierarchy(uint64_t folder_id,
    proptag_cspan pproptags, TARRAY_SET *pset)
{
	uint32_t row_num;
	uint32_t table_id;
	...
	for (size_t i = 0; i < tmp_set.count; ++i) {
		...
		if (pset->pparray[pset->count] == nullptr)
			return ecServerOOM;
		auto err = container_object_fetch_folder_properties(tmp_set.pparray[i],
...
ec_error_t container_object::query_container_table(proptag_cspan pproptags,
	...
	TARRAY_SET *pset)
{
	...
	if (CONTAINER_TYPE_FOLDER == pcontainer->type) {
		auto err = container_object_query_folder_hierarchy(
		...
		if (pcontainer->id.abtree_id.minid == ab_tree::minid::SC_ROOT) {
			...
			if (tmp_set.pparray[tmp_set.count] == nullptr)
				return ecServerOOM;
			auto err = container_object_fetch_special_properties(
			           ab_tree::minid::SC_GAL, pproptags,
			...
			if (tmp_set.pparray[tmp_set.count] == nullptr)
				return ecServerOOM;
			err = container_object_fetch_special_properties(
			      ab_tree::minid::SC_PROVIDER, pproptags,
			      tmp_set.pparray[tmp_set.count]);
			if (err != ecSuccess)
				return err;
			tmp_set.count ++;
			...
			if (!exmdb_client->get_folder_properties(pinfo->get_maildir(),
			    ...
			    container_object_get_folder_proptags, &tmp_propvals))
				return ecRpcFailed;
			err = container_object_fetch_folder_properties(&tmp_propvals,
			      pproptags, tmp_set.pparray[tmp_set.count]);
			if (err != ecSuccess)
				return err;
			tmp_set.count ++;
			if (b_depth) {
				err = container_object_query_folder_hierarchy(
...
ec_error_t container_object::get_user_table_num(uint32_t *pnum)
{
	auto pcontainer = this;
	
	if (type != CONTAINER_TYPE_ABTREE) {
		if (contents.prow_set == nullptr) {
			auto err = load_user_table(nullptr);
			if (err != ecSuccess)
				return err;
		}
...
ec_error_t container_object::query_user_table(proptag_cspan pproptags,
	uint32_t start_pos, int32_t row_needed, TARRAY_SET *pset)
{
	...
	if (CONTAINER_TYPE_ABTREE == pcontainer->type) {
		...
		if (pcontainer->contents.prow_set == nullptr) {
			auto err = pcontainer->load_user_table(nullptr);
			if (err != ecSuccess)
				return err;
		}
		if (pcontainer->contents.prow_set != nullptr)
