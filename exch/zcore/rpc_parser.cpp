...
static void *zcrp_thrwork(void *param)
{
	...
	while (true) {
		//Waiting for a client using a condition loop to prevent spurious wakeups
		{
			std::unique_lock<std::mutex> cm_hold(g_cond_mutex);
			while (double_list_get_nodes_num(&g_conn_list) == 0 && !g_notify_stop) {
				g_waken_cond.wait(cm_hold);
			}
			if (g_notify_stop)
				return nullptr;
		}
		//access and pop the next connection in separete block to prevent not needed unlocks
		{
			std::unique_lock<std::mutex> cl_hold(g_conn_lock);
			pnode = double_list_pop_front(&g_conn_list);
		}
		
		//instead of goto using while:continue loop
		if (pnode == nullptr)
			continue;
