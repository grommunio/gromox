...
#include "../exchange_nsp/nsp_types.h"
...
struct getproplist_request {
	uint32_t flags = 0, mid = 0, codepage = 0, cb_auxin = 0;
	uint8_t *auxin = nullptr;
};
...
struct getprops_response {
	uint32_t status = 0, result = 0, codepage = 0;
...
struct getspecialtable_response {
	uint32_t status = 0, result = 0, codepage = 0, count = 0;
	uint32_t *version = nullptr;
	LTPROPVAL_ARRAY *row = nullptr;
};
...
struct gettemplateinfo_request {
	...
	char *dn = nullptr;
	uint32_t codepage = 0, locale_id = 0, cb_auxin = 0;
	uint8_t *auxin = nullptr;
};
...
struct resolvenames_response {
	uint32_t status = 0, result = 0, codepage = 1200;
	MID_ARRAY *mids = nullptr;
	nsp_rowset2 column_rows{};
};
...
struct nsp_ext_pull : public EXT_PULL {
	int g_nsp_request(bind_request &);
	int g_nsp_request(unbind_request &);
	int g_nsp_request(comparemids_request &);
	int g_nsp_request(dntomid_request &);
	int g_nsp_request(getmatches_request &);
	int g_nsp_request(getproplist_request &);
	int g_nsp_request(getprops_request &);
	int g_nsp_request(getspecialtable_request &);
	int g_nsp_request(gettemplateinfo_request &);
	int g_nsp_request(modlinkatt_request &);
	int g_nsp_request(modprops_request &);
	int g_nsp_request(queryrows_request &);
	int g_nsp_request(querycolumns_request &);
	int g_nsp_request(resolvenames_request &);
	int g_nsp_request(resortrestriction_request &);
	int g_nsp_request(seekentries_request &);
	int g_nsp_request(updatestat_request &);
	int g_nsp_request(getmailboxurl_request &);
	int g_nsp_request(getaddressbookurl_request &);
};

struct nsp_ext_push : public EXT_PUSH {
	int p_nsp_response(const bind_response &);
	int p_nsp_response(const unbind_response &);
	int p_nsp_response(const comparemids_response &);
	int p_nsp_response(const dntomid_response &);
	int p_nsp_response(const getmatches_response &);
	int p_nsp_response(const getproplist_response &);
	int p_nsp_response(const getprops_response &);
	int p_nsp_response(const getspecialtable_response &);
	int p_nsp_response(const gettemplateinfo_response &);
	int p_nsp_response(const modlinkatt_response &);
	int p_nsp_response(const modprops_response &);
	int p_nsp_response(const queryrows_response &);
	int p_nsp_response(const querycolumns_response &);
	int p_nsp_response(const resolvenames_response &);
	int p_nsp_response(const resortrestriction_response &);
	int p_nsp_response(const seekentries_response &);
	int p_nsp_response(const updatestat_response &);
	int p_nsp_response(const getaddressbookurl_response &);
	int p_nsp_response(const getmailboxurl_response &);
};
