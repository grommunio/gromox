...
{
	NSP_HANDLE ses = {HANDLE_EXCHANGE_NSP, session_guid};
	return nsp_interface_compare_mids(ses, request.stat, request.mid1, request.mid2, &response.cmp);
}
...
{
	NSP_HANDLE ses = {HANDLE_EXCHANGE_NSP, session_guid};
	return nsp_interface_resort_restriction(ses,
    ...
    const updatestat_request &request, updatestat_response &response)
{
	...
	auto result = nsp_interface_update_stat(ses, request.stat, &delta);
	if (request.delta_requested != 0)
