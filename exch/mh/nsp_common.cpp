...
BOOL cu_entryid_to_binary(const nsp_entryid &eid, BINARY &bin)
{
	size_t len = 0;
	auto provider_uid = cu_guid_to_flatuid(eid.provider_uid);
	if (eid.id_type != 0x87) {
		len = strlen(eid.payload.dn) + 1;
		bin.cb = len + 28;
	} else {
		bin.cb = 32;
	}
	bin.pb = cu_alloc<uint8_t>(bin.cb);
	if (bin.pb == nullptr)
		return false;
	memset(bin.pb, 0, bin.cb);
	bin.pb[0] = eid.id_type;
	bin.pb[1] = eid.r1;
	bin.pb[2] = eid.r2;
	bin.pb[3] = eid.r3;
	memcpy(bin.pb + 4, provider_uid.ab, 16);
	bin.pb[20] = eid.r4 & 0xFF;
	bin.pb[21] = (eid.r4 >> 8) & 0xFF;
	bin.pb[22] = (eid.r4 >> 16) & 0xFF;
	bin.pb[23] = (eid.r4 >> 24) & 0xFF;
	bin.pb[24] = eid.display_type & 0xFF;
	bin.pb[25] = (eid.display_type >> 8)  & 0xFF;
	bin.pb[26] = (eid.display_type >> 16) & 0xFF;
	bin.pb[27] = (eid.display_type >> 24) & 0xFF;
	if (eid.id_type != 0x87) {
		memcpy(bin.pb + 28, eid.payload.dn, len);
		return TRUE;
	}
	bin.pb[28] = eid.payload.mid & 0xFF;
	bin.pb[29] = (eid.payload.mid >> 8)  & 0xFF;
	bin.pb[30] = (eid.payload.mid >> 16) & 0xFF;
	bin.pb[31] = (eid.payload.mid >> 24) & 0xFF;
	return TRUE;
}
