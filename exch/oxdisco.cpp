...
BOOL OxdiscoPlugin::preproc(int ctx_id)
{
    ...
    if (strcasecmp(uri, "/autodiscover/autodiscover.xml") != 0 &&
        strncasecmp(uri, "/.well-known/autoconfig/mail/config-v1.1.xml", 40) != 0 && strncasecmp(uri, "/autodiscover/autodiscover.json", 30) != 0)
        return false;
    return TRUE;
}
...
BOOL OxdiscoPlugin::proc(int ctx_id, const void *content, uint64_t len)
try
{
        ...
        strncasecmp(email, public_folder_email, 19) != 0)
    {
        ...
        if (std::none_of(hints.begin(), hints.end(),
                         [&](const sql_user &u)
                         { return strcasecmp(u.username.c_str(), email) == 0; }))
            /* authuser has no scndstore matching email */
            return die(ctx_id, bad_address_code, (bad_address_msg + " (403 Permission Denied)"s).c_str());
    }
...
/**
 * @brief      Create and send json response
 *
 * @param      ctx_id            Request context identifier
 * @param      get_request_uri   json get request uri
 *
...
static BOOL oxdisco_init(void **apidata)
{
    ...
    ifc.proc = [](int ctx, const void *cont, uint64_t len)
    { return g_oxdisco_plugin->proc(ctx, cont, len); };
    ifc.retr = [](int ctx)
