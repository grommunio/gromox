...
const std::unordered_map<std::string, EWSPlugin::Handler> EWSPlugin::requestMap = {
	...
	{"GetAppMarketplaceUrl", process<Structures::mGetAppMarketplaceUrlRequest>},
...
static constexpr cfg_directive ews_cfg_defaults[] = {
	...
	{"ews_streaming_subscription_timeout", "5min", CFG_TIME},
	CFG_TABLE_END,
};
...
void EWSPlugin::event(const char* dir, BOOL, uint32_t ID, const DB_NOTIFY* notification) const try
{
	...
	case db_notify_type::folder_modified: {
		/*
		 * Take the count from the notification. Calling
		 * exmdb.get_folder_properties() here would run the LPC wrapper
		 * on the exmdb parser thread, whose build_env/free_env
		 * destroys the arena owning the current request's dir pointer.
		 */
		tModifiedEvent event(now, mkFid(evt.folder_id),
			mkFid(evt.parent_id));
		if (evt.have_unread)
			event.UnreadCount = evt.unread;
		mgr->events.emplace_back(std::move(event));
		break;
	}
