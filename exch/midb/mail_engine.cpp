	...
	const char *mid_string, char *digest_buff)
{
	...
	snprintf(sql_string, arsizeof(sql_string), "SELECT uid, recent, read,"
...
static BOOL mail_engine_sync_contents(IDB_ITEM *pidb, uint64_t folder_id)
{
	...
	snprintf(sql_string, arsizeof(sql_string), "SELECT message_id, mid_string,"
	...
	if (0 != double_list_get_nodes_num(&temp_list)) {
		snprintf(sql_string, arsizeof(sql_string), "DELETE "
			"FROM messages WHERE message_id=?");
		pstmt = gx_sql_prep(pidb->psqlite, sql_string);
		if (pstmt == nullptr) {
			return FALSE;
...
static BOOL mail_engine_sync_mailbox(IDB_ITEM *pidb)
{
	...
	snprintf(sql_string, arsizeof(sql_string), "SELECT folder_id, parent_fid, "
...
static int mail_engine_mcopy(int argc, char **argv, int sockd)
{
	...
	write(sockd, mid_string.c_str(), mid_string.size());
	return 0;
}
...
static int mail_engine_pgflg(int argc, char **argv, int sockd)
{
	...
	snprintf(sql_string, arsizeof(sql_string), "SELECT folder_id, recent, "
