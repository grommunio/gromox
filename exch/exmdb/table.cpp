    ...
    const acsort &accel_pair, const BINARY *conv, bool b_private, bool b_search)
{
	...
	/*
	 * The message_id tiebreak matches the (folder_id, <ts>, message_id)
	 * index and the live-insert notification path
	 * (dbeng_notify_cttbl_add_row), so a fresh load reproduces exactly
	 * what incremental notifications built. Same direction as the primary
	 * key keeps the ORDER BY index-satisfiable.
	 */
   ...
   uint32_t *ptable_id, uint32_t *prow_count) try
{
	...
	if (accel_pair.dir != 0) {
		/*
		 * The accel/msgtime_index path loads without a stored sort
		 * order, but the live row-insert notification path needs to
		 * know the sort key to position new rows (rather than append
		 * at the bottom).
		 */
