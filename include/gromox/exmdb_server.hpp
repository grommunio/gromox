...
extern BOOL exmdb_server_create_folder_by_properties(const char *dir, uint32_t cpid, TPROPVAL_ARRAY *, uint64_t *folder_id);
...
BOOL exmdb_server_load_embedded_instance(const char *dir,
	...
	uint32_t *pinstance_id);
extern BOOL exmdb_server_get_embedded_cn(const char *dir, uint32_t instance_id, uint64_t **ppcn);
...
extern BOOL exmdb_server_flush_instance(const char *dir, uint32_t instance_id, const char *account, gxerr_t *);
