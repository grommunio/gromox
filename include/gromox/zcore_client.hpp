...
ZCIDL(notifdequeue, (const NOTIF_SINK &sink, uint32_t timeval, IDLOUT std::vector<ZNOTIFICATION> *notifications))
...
ZCIDL(link_messages, (GUID hsession, BINARY search_entryid, const BINARY_ARRAY *msg_eids))
...
ZCIDL(getdelegates, (GUID hsession, uint32_t mode, IDLOUT std::vector<std::string> *delegates))
#undef ZCIDL
#undef IDLOUT
