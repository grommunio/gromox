...
EXMIDL(create_folder_v1, (const char *dir, cpid_t cpid, TPROPVAL_ARRAY *pproperties, IDLOUT uint64_t *folder_id))
EXMIDL(create_folder, (const char *dir, cpid_t cpid, TPROPVAL_ARRAY *pproperties, IDLOUT uint64_t *folder_id, ec_error_t *e_result))
...
EXMIDL(write_message_v2, (const char *dir, const char *account, cpid_t cpid, uint64_t folder_id, const MESSAGE_CONTENT *pmsgctnt, IDLOUT uint64_t *outmid, uint64_t *outcn, ec_error_t *e_result))
