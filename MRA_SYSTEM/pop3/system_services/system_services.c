...
int (*system_services_list_mail)(char*, char*, ARRAY*, int *pnum, uint64_t *psize);
int (*system_services_delete_mail)(char*, char*, SINGLE_LIST*);
...
int system_services_run()
{
	...
	if (NULL == system_services_list_mail) {
		printf("[system_services]: fail to get \"list_mail\" service\n");
		return -8;
	}
