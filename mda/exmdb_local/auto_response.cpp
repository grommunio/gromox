...
void auto_response_reply(const char *user_home,
    const char *from, const char *rcpt, const char *orig_subject) try
{
	...
	/*
	 * EWS SetUserOofSettings has no field for a reply subject, so clients
	 * using it leave PR_EC_*_SUBJECT unset; derive one from the incoming
	 * message rather than emitting an empty Subject header.
	 */
	std::string subject_buf;
	if (*subject_text == '\0' && !autoreply_subject_prefix.empty()) {
		subject_buf = autoreply_subject_prefix;
		if (orig_subject != nullptr && *orig_subject != '\0') {
			subject_buf += orig_subject;
		} else {
