...
static int service_cmpstringp(const void *p1, const void *p2)
{
...
int service_run()
{
	...
	if (NULL == dirp){
		printf("[service]: fail to open plugins'"
				" directory %s\n", g_init_path);
		return -1;
	}
	...
	if (NULL == filename_array) {
		printf("[service]: fail to alloc memory for file name sorting\n");
		closedir(dirp);
		return -2;
	}
	seekdir(dirp,0);
	...
	qsort(filename_array, count, sizeof(char*), service_cmpstringp);
	for (i=0; i<count; i++) {
		sprintf(temp_path, "%s/%s", g_init_path, filename_array[i]);
