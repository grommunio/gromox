...
extern BOOL (*exmdb_client_get_named_propids)(const char *dir,
...
extern BOOL (*exmdb_client_get_named_propnames)(const char *dir,
	const PROPID_ARRAY *ppropids, PROPNAME_ARRAY *ppropnames);

extern BOOL (*exmdb_client_get_mapping_guid)(const char *dir,
	uint16_t replid, BOOL *pb_found, GUID *pguid);

extern BOOL (*exmdb_client_get_mapping_replid)(const char *dir,
	GUID guid, BOOL *pb_found, uint16_t *preplid);

extern BOOL (*exmdb_client_get_store_all_proptags)(
...
extern BOOL (*exmdb_client_check_mailbox_permission)(const char *dir,
	const char *username, uint32_t *ppermission);

extern BOOL (*exmdb_client_get_folder_by_class)(const char *dir,
	const char *str_class, uint64_t *pid, char *str_explicit);

extern BOOL (*exmdb_client_set_folder_by_class)(const char *dir,
	uint64_t folder_id, const char *str_class, BOOL *pb_result);

extern BOOL (*exmdb_client_get_folder_class_table)(
	const char *dir, TARRAY_SET *ptable);

extern BOOL (*exmdb_client_check_folder_id)(const char *dir,
	uint64_t folder_id, BOOL *pb_exist);

extern BOOL (*exmdb_client_check_folder_deleted)(const char *dir,
	uint64_t folder_id, BOOL *pb_del);

extern BOOL (*exmdb_client_get_folder_by_name)(const char *dir,
	uint64_t parent_id, const char *str_name,
	uint64_t *pfolder_id);

extern BOOL (*exmdb_client_check_folder_permission)(const char *dir,
	uint64_t folder_id, const char *username,
	uint32_t *ppermission);

extern BOOL (*exmdb_client_create_folder_by_properties)(const char *dir,
	uint32_t cpid, const TPROPVAL_ARRAY *pproperties,
	uint64_t *pfolder_id);

extern BOOL (*exmdb_client_get_folder_all_proptags)(const char *dir,
...
extern BOOL (*exmdb_client_remove_folder_properties)(const char *dir,
	uint64_t folder_id, const PROPTAG_ARRAY *pproptags);

extern BOOL (*exmdb_client_delete_folder)(const char *dir,
	uint32_t cpid, uint64_t folder_id, BOOL b_hard, BOOL *pb_result);

extern BOOL (*exmdb_client_empty_folder)(const char *dir,
	uint32_t cpid, const char *username, uint64_t folder_id,
	BOOL b_hard, BOOL b_normal, BOOL b_fai, BOOL b_sub,
	BOOL *pb_partial);

extern BOOL (*exmdb_client_check_folder_cycle)(const char *dir,
	uint64_t src_fid, uint64_t dst_fid, BOOL *pb_cycle);

extern BOOL (*exmdb_client_copy_folder_internal)(const char *dir,
...
extern BOOL (*exmdb_client_movecopy_folder)(const char *dir,
	int account_id, uint32_t cpid, BOOL b_guest, const char *username,
...
extern BOOL (*exmdb_client_delete_messages)(const char *dir,
...
extern BOOL (*exmdb_client_get_message_brief)(const char *dir,
	uint32_t cpid, uint64_t message_id, MESSAGE_CONTENT **ppbrief);

extern BOOL (*exmdb_client_sum_hierarchy)(const char *dir,
	uint64_t folder_id, const char *username,
	BOOL b_depth, uint32_t *pcount);
	
extern BOOL (*exmdb_client_load_hierarchy_table)(const char *dir,
	uint64_t folder_id, const char *username, uint8_t table_flags,
	const RESTRICTION *prestriction, uint32_t *ptable_id,
	uint32_t *prow_count);

extern BOOL (*exmdb_client_sum_content)(const char *dir, uint64_t folder_id,
	BOOL b_fai, BOOL b_deleted, uint32_t *pcount);

extern BOOL (*exmdb_client_load_content_table)(const char *dir,
	uint32_t cpid, uint64_t folder_id, const char *username,
	uint8_t table_flags, const RESTRICTION *prestriction,
	const SORTORDER_SET *psorts, uint32_t *ptable_id,
	uint32_t *prow_count);

extern BOOL (*exmdb_client_reload_content_table)(
	const char *dir, uint32_t table_id);

extern BOOL (*exmdb_client_load_permission_table)(const char *dir,
	uint64_t folder_id, uint8_t table_flags,
	uint32_t *ptable_id, uint32_t *prow_count);

extern BOOL (*exmdb_client_load_rule_table)(const char *dir,
	uint64_t folder_id, uint8_t table_flags,
	const RESTRICTION *prestriction,
	uint32_t *ptable_id, uint32_t *prow_count);

extern BOOL (*exmdb_client_unload_table)(const char *dir, uint32_t table_id);

extern BOOL (*exmdb_client_sum_table)(const char *dir,
	uint32_t table_id, uint32_t *prows);

extern BOOL (*exmdb_client_query_table)(const char *dir,
...
extern BOOL (*exmdb_client_match_table)(const char *dir,
	const char *username, uint32_t cpid, uint32_t table_id,
	BOOL b_forward, uint32_t start_pos, const RESTRICTION *pres,
	const PROPTAG_ARRAY *pproptags, int32_t *pposition,
	TPROPVAL_ARRAY *ppropvals);

extern BOOL (*exmdb_client_locate_table)(const char *dir,
	uint32_t table_id, uint64_t inst_id, uint32_t inst_num,
	int32_t *pposition, uint32_t *prow_type);

extern BOOL (*exmdb_client_read_table_row)(const char *dir,
	const char *username, uint32_t cpid, uint32_t table_id,
	const PROPTAG_ARRAY *pproptags, uint64_t inst_id,
	uint32_t inst_num, TPROPVAL_ARRAY *ppropvals);

extern BOOL (*exmdb_client_mark_table)(const char *dir,
	uint32_t table_id, uint32_t position, uint64_t *pinst_id,
	uint32_t *pinst_num, uint32_t *prow_type);

extern BOOL (*exmdb_client_get_table_all_proptags)(const char *dir,
	uint32_t table_id, PROPTAG_ARRAY *pproptags);

extern BOOL (*exmdb_client_expand_table)(const char *dir,
	uint32_t table_id, uint64_t inst_id, BOOL *pb_found,
	int32_t *pposition, uint32_t *prow_count);

extern BOOL (*exmdb_client_collapse_table)(const char *dir,
	uint32_t table_id, uint64_t inst_id, BOOL *pb_found,
	int32_t *pposition, uint32_t *prow_count);

extern BOOL (*exmdb_client_store_table_state)(const char *dir,
	uint32_t table_id, uint64_t inst_id, uint32_t inst_num,
	uint32_t *pstate_id);

extern BOOL (*exmdb_client_restore_table_state)(const char *dir,
	uint32_t table_id, uint32_t state_id, int32_t *pposition);

extern BOOL (*exmdb_client_check_message)(const char *dir,
	uint64_t folder_id, uint64_t message_id, BOOL *pb_exist);

extern BOOL (*exmdb_client_check_message_deleted)(const char *dir,
	uint64_t message_id, BOOL *pb_del);

extern BOOL (*exmdb_client_load_message_instance)(const char *dir,
...
extern BOOL (*exmdb_client_clear_message_instance)(
	const char *dir, uint32_t instance_id);

extern BOOL (*exmdb_client_read_message_instance)(const char *dir,
	uint32_t instance_id, MESSAGE_CONTENT *pmsgctnt);

extern BOOL (*exmdb_client_write_message_instance)(const char *dir,
	uint32_t instance_id, const MESSAGE_CONTENT *pmsgctnt,
	BOOL b_force, PROPTAG_ARRAY *pproptags,
	PROBLEM_ARRAY *pproblems);

extern BOOL (*exmdb_client_load_attachment_instance)(const char *dir,
	uint32_t message_instance_id, uint32_t attachment_num,
	uint32_t *pinstance_id);

extern BOOL (*exmdb_client_create_attachment_instance)(const char *dir,
	uint32_t message_instance_id, uint32_t *pinstance_id,
	uint32_t *pattachment_num);

extern BOOL (*exmdb_client_read_attachment_instance)(const char *dir,
	uint32_t instance_id, ATTACHMENT_CONTENT *pattctnt);

extern BOOL (*exmdb_client_write_attachment_instance)(const char *dir,
	uint32_t instance_id, const ATTACHMENT_CONTENT *pattctnt,
	BOOL b_force, PROBLEM_ARRAY *pproblems);

extern BOOL (*exmdb_client_delete_message_instance_attachment)(
...
extern BOOL (*exmdb_client_unload_instance)(
	const char *dir, uint32_t instance_id);

extern BOOL (*exmdb_client_get_instance_all_proptags)(
	const char *dir, uint32_t instance_id,
...
extern BOOL (*exmdb_client_get_instance_properties)(
...
extern BOOL (*exmdb_client_set_instance_properties)(
	const char *dir, uint32_t instance_id,
	const TPROPVAL_ARRAY *pproperties,
	PROBLEM_ARRAY *pproblems);
...
extern BOOL (*exmdb_client_remove_instance_properties)(
	const char *dir, uint32_t instance_id,
	const PROPTAG_ARRAY *pproptags, PROBLEM_ARRAY *pproblems);

extern BOOL (*exmdb_client_check_instance_cycle)(const char *dir,
	uint32_t src_instance_id, uint32_t dst_instance_id, BOOL *pb_cycle);

extern BOOL (*exmdb_client_empty_message_instance_rcpts)(
	const char *dir, uint32_t instance_id);

extern BOOL (*exmdb_client_get_message_instance_rcpts_num)(
	const char *dir, uint32_t instance_id, uint16_t *pnum);

extern BOOL (*exmdb_client_get_message_instance_rcpts_all_proptags)(
	const char *dir, uint32_t instance_id, PROPTAG_ARRAY *pproptags);

extern BOOL (*exmdb_client_get_message_instance_rcpts)(
	const char *dir, uint32_t instance_id, uint32_t row_id,
	uint16_t need_count, TARRAY_SET *pset);

extern BOOL (*exmdb_client_update_message_instance_rcpts)(
	const char *dir, uint32_t instance_id, const TARRAY_SET *pset);

extern BOOL (*exmdb_client_copy_instance_rcpts)(
	const char *dir, BOOL b_force, uint32_t src_instance_id,
	uint32_t dst_instance_id, BOOL *pb_result);

extern BOOL (*exmdb_client_empty_message_instance_attachments)(
	const char *dir, uint32_t instance_id);

extern BOOL (*exmdb_client_get_message_instance_attachments_num)(
	const char *dir, uint32_t instance_id, uint16_t *pnum);

extern BOOL (*exmdb_client_get_message_instance_attachment_table_all_proptags)(
	const char *dir, uint32_t instance_id, PROPTAG_ARRAY *pproptags);

extern BOOL (*exmdb_client_query_message_instance_attachment_table)(
	const char *dir, uint32_t instance_id, const PROPTAG_ARRAY *pproptags,
	uint32_t start_pos, int32_t row_needed, TARRAY_SET *pset);

extern BOOL (*exmdb_client_copy_instance_attachments)(
	const char *dir, BOOL b_force, uint32_t src_instance_id,
	uint32_t dst_instance_id, BOOL *pb_result);

extern BOOL (*exmdb_client_set_message_instance_conflict)(
	const char *dir, uint32_t instance_id, const MESSAGE_CONTENT *pmsgctnt);

extern BOOL (*exmdb_client_get_message_rcpts)(const char *dir,
...
extern BOOL (*exmdb_client_get_message_properties)(
	const char *dir, const char *username, uint32_t cpid,
...
extern BOOL (*exmdb_client_set_message_properties)(
	const char *dir, const char *username, uint32_t cpid,
	uint64_t message_id, const TPROPVAL_ARRAY *pproperties,
	PROBLEM_ARRAY *pproblems);

extern BOOL (*exmdb_client_set_message_read_state)(const char *dir,
	const char *username, uint64_t message_id, uint8_t mark_as_read,
	uint64_t *pread_cn);

BOOL exmdb_client_remove_message_property(const char *dir,
	uint32_t cpid, uint64_t message_id, uint32_t proptag);

extern BOOL (*exmdb_client_remove_message_properties)(
	const char *dir, uint32_t cpid, uint64_t message_id,
	const PROPTAG_ARRAY *pproptags);

extern BOOL (*exmdb_client_allocate_message_id)(const char *dir,
	uint64_t folder_id, uint64_t *pmessage_id);

extern BOOL (*exmdb_client_allocate_cn)(const char *dir, uint64_t *pcn);

extern BOOL (*exmdb_client_get_message_group_id)(const char *dir,
	uint64_t message_id, uint32_t **ppgroup_id);

extern BOOL (*exmdb_client_set_message_group_id)(const char *dir,
	uint64_t message_id, uint32_t group_id);

extern BOOL (*exmdb_client_save_change_indices)(const char *dir,
	uint64_t message_id, uint64_t cn, const INDEX_ARRAY *pindices,
	const PROPTAG_ARRAY *pungroup_proptags);

extern BOOL (*exmdb_client_get_change_indices)(const char *dir,
	uint64_t message_id, uint64_t cn, INDEX_ARRAY *pindices,
	PROPTAG_ARRAY *pungroup_proptags);

extern BOOL (*exmdb_client_mark_modified)(
	const char *dir, uint64_t message_id);

extern BOOL (*exmdb_client_try_mark_submit)(const char *dir,
	uint64_t message_id, BOOL *pb_marked);

extern BOOL (*exmdb_client_clear_submit)(const char *dir,
	uint64_t message_id, BOOL b_unsent);

extern BOOL (*exmdb_client_link_message)(const char *dir,
	uint32_t cpid, uint64_t folder_id, uint64_t message_id,
	BOOL *pb_result);

extern BOOL (*exmdb_client_unlink_message)(const char *dir,
	uint32_t cpid, uint64_t folder_id, uint64_t message_id);

extern BOOL (*exmdb_client_rule_new_message)(const char *dir,
	const char *username, const char *account, uint32_t cpid,
	uint64_t folder_id, uint64_t message_id);

extern BOOL (*exmdb_client_set_message_timer)(const char *dir,
	uint64_t message_id, uint32_t timer_id);

extern BOOL (*exmdb_client_get_message_timer)(const char *dir,
	uint64_t message_id, uint32_t **pptimer_id);

extern BOOL (*exmdb_client_empty_folder_permission)(
	const char *dir, uint64_t folder_id);

extern BOOL (*exmdb_client_update_folder_permission)(const char *dir,
	uint64_t folder_id, BOOL b_freebusy,
	uint16_t count, const PERMISSION_DATA *prow);

extern BOOL (*exmdb_client_empty_folder_rule)(
	const char *dir, uint64_t folder_id);

extern BOOL (*exmdb_client_update_folder_rule)(const char *dir,
	uint64_t folder_id, uint16_t count,
	const RULE_DATA *prow, BOOL *pb_exceed);

extern BOOL (*exmdb_client_delivery_message)(const char *dir,
	const char *from_address, const char *account,
	uint32_t cpid, const MESSAGE_CONTENT *pmsg,
	const char *pdigest, uint32_t *presult);
extern BOOL (*exmdb_client_write_message)(const char *dir, const char *account, uint32_t cpid, uint64_t folder_id, const MESSAGE_CONTENT *, gxerr_t *);
extern BOOL (*exmdb_client_read_message)(const char *dir,
	const char *username, uint32_t cpid, uint64_t message_id,
	MESSAGE_CONTENT **ppmsgctnt);

extern BOOL (*exmdb_client_get_content_sync)(const char *dir,
...
extern BOOL (*exmdb_client_get_hierarchy_sync)(const char *dir,
	uint64_t folder_id, const char *username, const IDSET *pgiven,
	const IDSET *pseen, FOLDER_CHANGES *pfldchgs, uint64_t *plast_cn,
	EID_ARRAY *pgiven_fids, EID_ARRAY *pdeleted_fids);

extern BOOL (*exmdb_client_allocate_ids)(const char *dir,
	uint32_t count, uint64_t *pbegin_eid);

extern BOOL (*exmdb_client_subscribe_notification)(const char *dir,
	uint16_t notificaton_type, BOOL b_whole, uint64_t folder_id,
	uint64_t message_id, uint32_t *psub_id);

extern BOOL (*exmdb_client_unsubscribe_notification)(
	const char *dir, uint32_t sub_id);

extern BOOL (*exmdb_client_transport_new_mail)(const char *dir,
