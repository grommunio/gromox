...
BOOL bounce_producer_refresh()
{
	...
	dirp = opendir(g_path);
	if (NULL == dirp) {
		printf("[exmdb_provider]: failed to open directory %s: %s\n",
	...
	if (NULL == pdefault) {
		printf("[exmdb_provider]: there's no "
			"\"ascii\" bounce mail templates\n");
...
static void bounce_producer_load_subdir(const char *dir_name, SINGLE_LIST *plist)
{
	...
	if (NULL == presource) {
		printf("[exmdb_provider]: fail to allocate resource node memory\n");
		return;
	}
