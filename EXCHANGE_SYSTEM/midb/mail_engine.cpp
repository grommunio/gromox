	...
	BOOL b_uid)
{
	...
	sprintf(sql_string, "SELECT mid_string, uid FROM "
...
static BOOL mail_engine_sync_contents(IDB_ITEM *pidb, uint64_t folder_id)
{
	...
	sprintf(sql_string, "INSERT INTO messages (message_id, "
		...
		"?, ?, ?, ?, ?, ?)", LLU(folder_id));
	if (!gx_sql_prep(pidb->psqlite, sql_string, &pstmt2)) {
		sqlite3_finalize(pstmt);
		sqlite3_finalize(pstmt1);
		sqlite3_close(psqlite);
		return FALSE;
	...
	if (!gx_sql_prep(pidb->psqlite, sql_string, &pstmt3)) {
		sqlite3_finalize(pstmt);
		sqlite3_finalize(pstmt1);
...
static int mail_engine_mquta(int argc, char **argv, int sockd)
{
	...
	temp_len = sprintf(temp_string, "TRUE %llu %u %llu %d\r\n",
	           LLU(*ptotal), *pcount, LLU(quota), 0x7FFFFFFF);
...
static int mail_engine_mlist(int argc, char **argv, int sockd)
{
	...
	if (TRUE == b_asc) {
		...
		idx2 = idx1 + length - 1;
		sprintf(sql_string, "SELECT mid_string FROM messages "
...
static int mail_engine_muidl(int argc, char **argv, int sockd)
{
	...
	while (SQLITE_ROW == (result = sqlite3_step(pstmt))) {
		pinode = static_cast<IDL_NODE *>(common_util_alloc(sizeof(IDL_NODE)));
		if (NULL == pinode) {
			sqlite3_finalize(pstmt);
			mail_engine_put_idb(pidb);
			return 4;
...
static int mail_engine_mupdt(int argc, char **argv, int sockd)
{
	...
	if (0 == strcasecmp(argv[4], "recent")) {
		...
		sprintf(sql_string, "UPDATE messages SET recent=%d"
		        " WHERE message_id=%llu", tmp_val, LLU(message_id));
		sqlite3_exec(pidb->psqlite, sql_string, NULL, NULL, NULL);
		...
		pext = S2A(sqlite3_column_text(pstmt, 0));
		if (NULL == pext) {
		...
		sprintf(sql_string, "UPDATE messages SET"
		          " ext=? WHERE message_id=%llu", LLU(message_id));
		if (!gx_sql_prep(pidb->psqlite, sql_string, &pstmt)) {
			mail_engine_put_idb(pidb);
			return 4;
		}
...
static int mail_engine_pfddt(int argc, char **argv, int sockd)
{
	...
	sqlite3_finalize(pstmt);
	sprintf(sql_string, "SELECT min(idx) FROM messages "
...
static int mail_engine_psiml(int argc, char **argv, int sockd)
{
	...
	if (TRUE == b_asc) {
		...
		idx2 = idx1 + length - 1;
		sprintf(sql_string, "SELECT mid_string, uid, replied, "
...
static int mail_engine_psimu(int argc, char **argv, int sockd)
{
	...
	if (TRUE == b_asc) {
		if (-1 == first && -1 == last) {
			sprintf(sql_string, "SELECT idx, mid_string, uid, "
	...
	double_list_init(&temp_list);
	while (SQLITE_ROW == sqlite3_step(pstmt)) {
		auto psm_node = static_cast<SIMU_NODE *>(common_util_alloc(sizeof(SIMU_NODE)));
		if (NULL == psm_node) {
			sqlite3_finalize(pstmt);
			mail_engine_put_idb(pidb);
			return 4;
...
static int mail_engine_pdell(int argc, char **argv, int sockd)
{
	...
	sqlite3_finalize(pstmt);
	if (TRUE == b_asc) {
		sprintf(sql_string, "SELECT idx, mid_string, uid FROM"
...
static int mail_engine_pdtlu(int argc, char **argv, int sockd)
{
	...
	if (TRUE == b_asc) {
		if (-1 == first && -1 == last) {
			sprintf(sql_string, "SELECT idx, mid_string"
	...
	IDB_ITEM *pidb, uint64_t folder_id, uint64_t message_id)
{
	...
	sprintf(sql_string, "INSERT INTO messages ("
