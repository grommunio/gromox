...
function ModifyItem(domain, part) {\
opeform.domain.value=domain; opeform.part.value=part; \
opeform.part.focus();}\n\
...
static void	table_ui_modify_list(const char *domain, const char *part);
...
int table_ui_run()
{
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			table_ui_modify_list(temp_domain, part);
			table_ui_main_html(session);
			return 0;
		}
	} else {
...
static void table_ui_broadcast_list()
{
	...
	int i, len;
	int fd, fd_domain;
	int item_num, dns_num;
	char temp_line[256];
	char temp_path[256];
	char temp_domain[128];
	struct dirent *direntp;
	LIST_FILE *pfile, *pfile_dns;
	DNS_ITEM *pdns;
	TABLE_ITEM *pitem;
	...
	pfile_dns = list_file_init(g_dns_path, "%s:256:%s:8%s:256");
	if (NULL == pfile_dns) {
		system_log_info("[table_ui]: fail to open list file %s",
			g_dns_path);
		list_file_free(pfile);
	...
	item_num = list_file_get_item_num(pfile);
	pitem = (TABLE_ITEM*)list_file_get_list(pfile);
	dns_num = list_file_get_item_num(pfile_dns);
	pdns = (DNS_ITEM*)list_file_get_list(pfile_dns);
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/redirect_domains.txt",
			g_mount_path, direntp->d_name);
		fd_domain = open(temp_path, O_CREAT|O_TRUNC|O_WRONLY, DEF_MODE);
		if (-1 == fd_domain) {
			system_log_info("[table_ui]: fail to truncate %s", temp_path);
			close(fd);
			continue;
		...
		for (i=0; i<item_num; i++) {
			...
			len = sprintf(temp_line, "MX\t%s\t%s\n", temp_domain, pitem[i].part);
			write(fd, temp_line, len);
			len = sprintf(temp_line, "%s\n", temp_domain);
			write(fd_domain, temp_line, len);
		}
		close(fd_domain);
		for (i=0; i<dns_num; i++) {
			table_ui_encode_line(pdns[i].domain, temp_domain);
			if (0 == strcmp(pdns[i].type, "MX")) {
				len = sprintf(temp_line, "MX\t%s\t%s\n", temp_domain,
						pdns[i].part);
			} else {
