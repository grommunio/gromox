...
#define DNS_RELOAD_COMMAND		"dns_adaptor.svc reload fixed\r\n"
#define DOMAIN_RELOAD_COMMAND	"redirect_domains.svc reload\r\n"
...
static BOOL reload_control_send(const char *ip, int port)
{
	...
	/* send command */
	write(sockd, DNS_RELOAD_COMMAND, sizeof(DNS_RELOAD_COMMAND) - 1);
	read(sockd, temp_buff, 1024);
	write(sockd, DOMAIN_RELOAD_COMMAND, sizeof(DOMAIN_RELOAD_COMMAND) - 1);
	write(sockd, "quit\r\n", 6);
	close(sockd);
	return TRUE;
}
