	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		while (NULL != fgets(temp_buff, 64*1024, fp)) {
			...
			if (NULL != strstr(ptr, "illegal mail is cut!")) {
				spam_num ++;
			} else if (NULL != strstr(ptr, "flushing queue permanent fail")) {
				spam_num ++;
			} else if (NULL != strstr(ptr, "dubious mail is cut!")) {
				spam_num ++;
			} else {
				continue;
			}
		...
		while (NULL != fgets(temp_buff, 64*1024, fp)) {
			...
			if (NULL != strstr(ptr, " back-end server ") &&
				NULL != strstr(ptr, " return OK")) {
				normal_num ++;
			}
		}
		fclose(fp);
