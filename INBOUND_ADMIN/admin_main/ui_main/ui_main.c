...
static void ui_main_left_html(const char *session, BOOL b_ext)
{
	...
	if (TRUE == b_ext) {	
		...
		printf("p.add(3,0,'%s','admin_invalid?session=%s','','','','','','','basefrm');\n",
			lang_resource_get(g_lang_resource,"MENU_ADMIN_INVALID", language), session);
		...
		printf("p.add(6,4,'%s','synchronization_allow?session=%s','','','','','','','basefrm');\n",
			lang_resource_get(g_lang_resource,"MENU_SYNCHRONIZATION_ALLOW", language), session);
		printf("p.add(7,4,'%s','local_setup?session=%s','','','','','','','basefrm');\n",
			lang_resource_get(g_lang_resource,"MENU_LOCAL_SETUP", language), session);
		printf("p.add(8,4,'%s','backend_table?session=%s','','','','','','','basefrm');\n",
			lang_resource_get(g_lang_resource,"MENU_BACKEND_TABLE", language), session);
		printf("p.add(9,4,'%s','relay_table?session=%s','','','','','','','basefrm');\n",
			lang_resource_get(g_lang_resource,"MENU_RELAY_TABLE", language), session);
		printf("p.add(10,4,'%s','relay_allow?session=%s','','','','','','','basefrm');\n",
		...
		printf(HTML_LEFT_ITEM_SUB, "admin_invalid", session,
			lang_resource_get(g_lang_resource,"MENU_ADMIN_INVALID", language));
		...
		printf(HTML_LEFT_ITEM_SUB, "synchronization_allow", session,
			lang_resource_get(g_lang_resource,"MENU_SYNCHRONIZATION_ALLOW", language));
		...
		printf(HTML_LEFT_ITEM_SUB, "redirect_table", session,
			lang_resource_get(g_lang_resource,"MENU_RELAY_DOMAINS", language));
		printf(HTML_LEFT_ITEM_SUB, "relay_domains", session,
			lang_resource_get(g_lang_resource,"MENU_REDIRECT_TABLE", language));
