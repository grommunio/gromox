...
opeform.ip.value=ip; opeform.memo.value=memo; \
opeform.memo.focus();}\n\
...
static void	list_ui_modify_list(const char *ip, const char *memo);

static void list_ui_remove_item(const char *ip);
...
int list_ui_run()
{
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			list_ui_modify_list(ip_addr, memo);
			list_ui_main_html(session);
			return 0;
		}
	} else {
...
static void list_ui_main_html(const char *session)
{
	...
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, pitem[i].ip,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language), pitem[i].ip,
			...
			printf(HTML_TBITEM_NORMAL, pitem[i].ip, pitem[i].memo, pitem[i].ip, 
				lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].ip, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		}
...
static void list_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/relay_allow.txt",
