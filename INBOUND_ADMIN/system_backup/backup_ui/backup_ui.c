...
static BOOL backup_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[backup_ui]: fail to get SERVER_NAME or "
...
BACKEND_TABLE:
	sprintf(src_file, "%s/data_files/backend_table.txt", g_backup_path);
	sprintf(dst_file, "%s/backend_table.txt", g_data_path);
...
DNS_TABLE:
	...
	sprintf(src_file, "%s/data_files/redirect_table.txt", g_backup_path);
	sprintf(dst_file, "%s/redirect_table.txt", g_data_path);
	sprintf(temp_file1, "%s/data_files/temp1.txt", g_backup_path);
	file_operation_copy_file(src_file, dst_file);
...
LOCAL_SETUP:
	sprintf(src_file, "%s/data_files/local_setup.txt", g_backup_path);
	sprintf(dst_file, "%s/local_setup.txt", g_data_path);
	file_operation_copy_file(src_file, dst_file);
	file_operation_broadcast(src_file, "data/delivery/inbound_ips.txt");
	sprintf(src_file, "%s/data_files/domain_list.txt", g_backup_path);
	sprintf(dst_file, "%s/domain_list.txt", g_data_path);
	file_operation_copy_file(src_file, dst_file);
	file_operation_broadcast(src_file, "data/smtp/domain_list.txt");
	file_operation_broadcast(src_file, "data/delivery/domain_list.txt");

DOMAIN_ACL:
	sprintf(src_file, "%s/data_files/domain_acl.txt", g_backup_path);
	sprintf(dst_file, "%s/domain_acl.txt", g_data_path);
	file_operation_copy_file(src_file, dst_file);

SYNCHRONIZATION_ALLOW:
	sprintf(src_file, "%s/data_files/synchronization_allow.txt", g_backup_path);
	sprintf(dst_file, "%s/synchronization_allow.txt", g_data_path);
	file_operation_copy_file(src_file, dst_file);
...
SYSTEM_SETUP:
	...
	str_value1 = config_file_get_value(pconfig, "LOCAL_SETUP_TYPE");
	if (NULL != str_value1) {
		switch (atoi(str_value1)) {
		case 0:
			gateway_control_notify("system set domain-list FALSE",
				NOTIFY_SMTP|NOTIFY_DELIVERY);
			break;
		case 1:
			gateway_control_notify("system set domain-list TRUE",
				NOTIFY_SMTP|NOTIFY_DELIVERY);
			break;
		case 2:
			gateway_control_notify("system set domain-list TRUE",
	...
	str_value1 = config_file_get_value(pconfig, "ACCOUNTENUM_TIMES");
	str_value2 = config_file_get_value(pconfig, "ACCOUNTENUM_INTERVAL_NUM");
	str_value3 = config_file_get_value(pconfig, "ACCOUNTENUM_INTERVAL_UNIT");
	if (NULL != str_value1 && NULL != str_value2 && NULL != str_value3) {
		unit_num = atoi(str_value3);
		switch (unit_num) {
		case UNIT_SECOND:
			sprintf(temp_line, "gateway_norcpt_audit.svc audit set %s/%s",
				str_value1, str_value2);
	...
	str_value1 = config_file_get_value(pconfig, "IPENUM_TIMES");
	str_value2 = config_file_get_value(pconfig, "IPENUM_INTERVAL_NUM");
	str_value3 = config_file_get_value(pconfig, "IPENUM_INTERVAL_UNIT");
	if (NULL != str_value1 && NULL != str_value2 && NULL != str_value3) {
		unit_num = atoi(str_value3);
		switch (unit_num) {
		case UNIT_SECOND:
			sprintf(temp_line, "gateway_nouser_audit.svc audit set %s/%s",
				str_value1, str_value2);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			break;
		case UNIT_MINUTE:
	...
	str_value1 = config_file_get_value(pconfig, "DISPATCH_RETRING_TIMES");
	str_value2 = config_file_get_value(pconfig, "DISPATCH_RETRING_INTERVAL_NUM");
	str_value3 = config_file_get_value(pconfig, "DISPATCH_RETRING_INTERVAL_UNIT");
	if (NULL != str_value1 && NULL != str_value2 && NULL != str_value3) {
		unit_num = atoi(str_value3);
		switch (unit_num) {
		case UNIT_SECOND:
			sprintf(temp_line, "gateway_dispatch.hook set retrying-times %s",
				str_value1);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			sprintf(temp_line, "gateway_dispatch.hook set cache-scan %s",
				str_value2);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			break;
		case UNIT_MINUTE:
			sprintf(temp_line, "gateway_dispatch.hook set retrying-times %s",
				str_value1);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			sprintf(temp_line, "gateway_dispatch.hook set cache-scan %sminutes",
				str_value2);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			break;
		case UNIT_HOUR:
			sprintf(temp_line, "gateway_dispatch.hook set retrying-times %s",
				str_value1);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			sprintf(temp_line, "gateway_dispatch.hook set cache-scan %shours",
				str_value2);
			gateway_control_notify(temp_line, NOTIFY_DELIVERY);
			break;
		}
	...
	str_value1 = config_file_get_value(pconfig, "GATEWAY_BOUNCE_POLICY");
	if (NULL != str_value1) {
		sprintf(temp_line, "gateway_dispatch.hook set bounce-policy %s",
			str_value1);
		gateway_control_notify(temp_line, NOTIFY_DELIVERY);
	}
	...
	str_value1 = config_file_get_value(pconfig, "HASH_HUGE");
	if (NULL != str_value1) {
		if (0 == strcasecmp(str_value1, "TRUE")) {
			strcpy(temp_line, "mail_backup.hook set hash-huge TRUE");
		} else {
			strcpy(temp_line, "mail_backup.hook set hash-huge FALSE");
		}
		gateway_control_notify(temp_line, NOTIFY_DELIVERY);
	}
...
static void backup_ui_backup_file()
{
	...
	sprintf(src_file, "%s/local_setup.txt", g_data_path);
	sprintf(dst_file, "%s/data_files/local_setup.txt", g_backup_path);
	file_operation_copy_file(src_file, dst_file);
	...
	sprintf(src_file, "%s/domain_list.txt", g_data_path);
	sprintf(dst_file, "%s/data_files/domain_list.txt", g_backup_path);
	file_operation_copy_file(src_file, dst_file);
