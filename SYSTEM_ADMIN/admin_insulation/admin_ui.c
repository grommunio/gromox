...
int admin_ui_run()
{
	...
	if (NULL == request) {
		system_log_info("[admin_ui]: $REQUEST_METHOD is unset");
		return -2;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[admin_ui]: $REMOTE_ADDR is unset");
		return -3;
	}
	if (0 == strcmp(request, "POST")) {
		...
		if (NULL == query) {
			system_log_info("[admin_ui]: $QUERY_STRING is unset");
			admin_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",language));
			return 0;
		} else {
			len = strlen(query);
...
static BOOL admin_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[admin_ui]: $HTTP_HOST or $SCRIPT_NAME is unset");
		return FALSE;
	}
