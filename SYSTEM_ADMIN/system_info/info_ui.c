...
int info_ui_run()
{
	...
	if (NULL == request) {
		system_log_info("[info_ui]: $REQUEST_METHOD is unset");
		return -2;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[info_ui]: $REMOTE_ADDR is unset");
		return -3;
	}
	if (0 == strcmp(request, "GET")) {
		query = getenv("QUERY_STRING");
		if (NULL == query) {
			system_log_info("[info_ui]: $QUERY_STRING is unset");
...
static BOOL info_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[info_ui]: $HTTP_HOST or $SCRIPT_NAME is unset");
		return FALSE;
	}
