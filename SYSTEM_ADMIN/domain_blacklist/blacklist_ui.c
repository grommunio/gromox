...
static void	blacklist_ui_modify_list(const char *domain, const char *memo);

static void blacklist_ui_remove_item(const char *domain);
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			if (NULL == ptr2) {
				...
				blacklist_ui_remove_item(temp_domain);
				blacklist_ui_main_html(session);
				return 0;
			}
			...
			blacklist_ui_modify_list(temp_domain, memo);
			blacklist_ui_main_html(session);
			return 0;
		}
	} else {
...
static void blacklist_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/forbidden_domain.txt",
