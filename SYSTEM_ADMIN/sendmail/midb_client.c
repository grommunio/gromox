	...
	const char *messge_id, const char *dst_folder)
{
	...
	len = snprintf(temp_buff, 1024, "M-UPDT %s %s %s timesend 0\r\n",
			maildir, src_folder, messge_id);
	...
	len = snprintf(temp_buff, 1024, "M-UPDT %s %s %s randsendtime 0\r\n",
			maildir, src_folder, messge_id);
	if (len != write(sockd, temp_buff, len)) {
		close(sockd);
		return FALSE;
	...
	if (0 != strcmp(src_folder, dst_folder)) {
		len = snprintf(temp_buff, 1024, "M-MOVE %s %s %s %s\r\n", maildir,
				src_folder, messge_id, dst_folder);
		if (len != write(sockd, temp_buff, len)) {
			close(sockd);
			return FALSE;
		}
