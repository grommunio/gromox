...
function DeleteItem(original) {location.href='%s?session=%s&original=' + original;}\n\
function ModifyItem(original, replacing, memo) {\
opeform.original.value=original; opeform.replacing.value=replacing;\n\
...
name=original /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text \n\
value=\"\" tabindex=2 name=replacing /></TD></TR>\n\
...
static void	list_ui_modify_list(const char *original, const char *replacing,
	const char *memo);

static void list_ui_remove_item(const char *original);
...
int list_ui_run()
{
	...
	char memo[256];
	char session[256];
	char temp_original[256];
	char temp_replacing[256];
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			ptr1 += 8;
			ptr2 = search_string(search_buff, "&original=", len);
			if (NULL == ptr2) {
			...
			ptr1 = ptr2 + 10;
			ptr2 = search_string(search_buff, "&replacing=", len);
			if (NULL == ptr2) {
				if (search_buff + len - ptr1 - 1 > 256 ||
				...
				list_ui_remove_item(temp_original);
				list_ui_main_html(session);
				return 0;
			}
			...
			list_ui_modify_list(temp_original, temp_replacing, memo);
			list_ui_main_html(session);
			return 0;
		}
	} else {
...
static void list_ui_main_html(const char *session)
{
	...
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, pitem[i].original, pitem[i].replacing,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language),
				pitem[i].original, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].original, pitem[i].replacing, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		} else {
			printf(HTML_TBITEM_NORMAL, pitem[i].original, pitem[i].replacing,
				pitem[i].memo, pitem[i].original,
	...
	const char *memo)
{
	...
	item_num = list_file_get_item_num(pfile);
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(original, pitem[i].original)) {
			break;
		}
	}
...
static void list_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/from_replace.txt",
