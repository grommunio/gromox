...
int setup_ui_run()
{
	...
	if (g_lang_resource == nullptr) {
		system_log_info("[setup_ui]: failed to init language resource");
		return -1;
	}
	...
	if (NULL == request) {
		system_log_info("[setup_ui]: $REQUEST_METHOD is unset");
		return -1;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[setup_ui]: $REMOTE_ADDR is unset");
		return -2;
	}
	if (0 == strcmp(request, "GET")) {
		query = getenv("QUERY_STRING");
		if (NULL == query) {
			system_log_info("[setup_ui]: $QUERY_STRING is unset");
...
static BOOL setup_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[setup_ui]: $HTTP_HOST or $SCRIPT_NAME is unset");
		return FALSE;
	}
