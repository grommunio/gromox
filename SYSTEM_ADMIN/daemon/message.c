	...
	const char *str_domain_user, const char *admin_mailbox)
{
	...
	ptr = buff;
	len = sprintf(ptr, MESSAGE_BODY_1, lang_resource_get(g_lang_resource,"FROM_NAME",
			language));
	ptr += len;
	
	len = strlen(admin_mailbox);
	memcpy(ptr, admin_mailbox, len);
	ptr += len;
	...
	if (MESSAGE_TURN_OUTOFDATE == type) {
		len = sprintf(ptr, MESSAGE_BODY_3,
				lang_resource_get(g_lang_resource,"SUBJECT_TURN_OUTOFDATE", language),
				lang_resource_get(g_lang_resource,"CHARSET", language), 
				lang_resource_get(g_lang_resource,"HTML_TITLE", language),
				lang_resource_get(g_lang_resource,"HTML_TITLE", language), g_logo_link);
	} else if (MESSAGE_WILL_OUTOFDATE == type) {
		len = sprintf(ptr, MESSAGE_BODY_3,
				lang_resource_get(g_lang_resource,"SUBJECT_WILL_OUTOFDATE", language),
				lang_resource_get(g_lang_resource,"CHARSET", language), 
				lang_resource_get(g_lang_resource,"HTML_TITLE", language),
				lang_resource_get(g_lang_resource,"HTML_TITLE", language), g_logo_link);
	} else if (MESSAGE_PASSWORD_AGING == type) {
		len = sprintf(ptr, MESSAGE_BODY_3,
				lang_resource_get(g_lang_resource,"SUBJECT_PASSWORD_AGING", language),
				lang_resource_get(g_lang_resource,"CHARSET", language), 
				lang_resource_get(g_lang_resource,"HTML_TITLE", language),
				lang_resource_get(g_lang_resource,"HTML_TITLE", language), g_logo_link);
	}
	...
	if (MESSAGE_TURN_OUTOFDATE == type) {
		len = sprintf(ptr, lang_resource_get(g_lang_resource,"CONTENT_TURN_OUTOFDATE",
				language), str_domain_user);
	} else if (MESSAGE_WILL_OUTOFDATE == type){
		len = sprintf(ptr, lang_resource_get(g_lang_resource,"CONTENT_WILL_OUTOFDATE",
				language), str_domain_user);
	} else if (MESSAGE_PASSWORD_AGING == type) {
		len = sprintf(ptr, lang_resource_get(g_lang_resource,"CONTENT_PASSWORD_AGING",
				language), str_domain_user);
	}
	ptr += len;
