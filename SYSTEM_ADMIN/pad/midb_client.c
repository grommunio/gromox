	...
	const char *folder, const char *mid_string)
{
	...
	len = snprintf(temp_buff, 1024*1025, "M-INST %s %s %s () %ld\r\n",
							maildir, folder, mid_string, time(NULL));
...
BOOL midb_client_ckfl(const char *maildir)
{
	int len;
	int sockd;
	BACK_SVR *pserver;
	...
	len = snprintf(temp_buff, 1024*1025, "M-CKFL %s\r\n", maildir);
	if (len != write(sockd, temp_buff, len)) {
		close(sockd);
	}
	...
	write(sockd, "QUIT\r\n", 6);
	close(sockd);
	if (0 == strcasecmp(temp_buff, "TRUE 1")) {
		return FALSE;
	} else {
		return TRUE;
	}
