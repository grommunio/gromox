...
function ModifyItem(ip, memo) {\
opeform.ip.value=ip; opeform.memo.value=memo; opeform.memo.focus();}\n\
...
name=ip /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text value=\"\" \n\
tabindex=2 name=memo /></TD></TR><TR><TD></TD><TD></TD><TD>\n\
...
static void	blacklist_ui_modify_list(const char *ip, const char *memo);

static void blacklist_ui_remove_item(const char *ip);
	...
	const char *mount_path, const char *url_link, const char *resource_path)
{
	...
	strcpy(g_white_path, white_path);
		...
		if (NULL == query) {
			...
			if (NULL == ptr2) {
				...
				if (NULL == extract_ip(temp_ip, ip_addr)) {
					system_log_info("[blacklist_ui]: ip address in GET query "
						"string error");
					blacklist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",
						language));
					return 0;
				...
				blacklist_ui_remove_item(ip_addr);
				blacklist_ui_main_html(session);
				return 0;
			}
			...
			blacklist_ui_modify_list(ip_addr, memo);
			blacklist_ui_main_html(session);
			return 0;
		}
	} else {
...
static void blacklist_ui_main_html(const char *session)
{
	...
	pfile = list_file_init(g_list_path, "%s:16%l%s:256");
	if (NULL == pfile) {
	...
	for (i=0; i<item_num; i++) {
		...
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].ip,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language),
				pitem[i].ip, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].ip, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		} else {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].ip, pitem[i].memo,
				pitem[i].ip, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].ip, pitem[i].memo, lang_resource_get(g_lang_resource,"MODIFY_LABEL", 
				language));
		}
	}
...
static void blacklist_ui_broadcast_list()
{
	...
	LIST_FILE *pfile;
	LIST_FILE *pfile_white;
	BLACKLIST_ITEM *pitem;
	WHITELIST_ITEM *pwhite;
	
	
	pfile = list_file_init(g_list_path, "%s:16%l%s:256");
	pfile_white = list_file_init(g_white_path, "%s:16%d%s:4%s:256");
	if (NULL == pfile) {
		system_log_info("[blacklist_ui]: fail to open blacklist %s",
			g_list_path);
		return;
	}
	if (NULL == pfile_white) {
	...
	item_num = list_file_get_item_num(pfile);
	pitem = (BLACKLIST_ITEM*)list_file_get_list(pfile);
	white_num = list_file_get_item_num(pfile_white);
