...
static void	whitelist_ui_modify_list(const char *domain, const char *memo);

static void whitelist_ui_remove_item(const char *domain);
...
int whitelist_ui_run()
{
	...
	if (g_lang_resource == nullptr) {
		system_log_info("[whitelist_ui]: failed to init language resource");
		return -1;
	}
	...
	if (NULL == request) {
		system_log_info("[whitelist_ui]: $REQUEST_METHOD is unset");
		return -2;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[whitelist_ui]: $REMOTE_ADDR is unset");
		return -3;
	}
	if (0 == strcmp(request, "GET")) {
		query = getenv("QUERY_STRING");
		if (NULL == query) {
			system_log_info("[whitelist_ui]: $QUERY_STRING is unset");
			...
			if (NULL == ptr2) {
				...
				whitelist_ui_remove_item(temp_domain);
				whitelist_ui_main_html(session);
				return 0;
			}
			...
			whitelist_ui_modify_list(temp_domain, memo);
			whitelist_ui_main_html(session);
			return 0;
		}
	} else {
...
static BOOL whitelist_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[whitelist_ui]: $HTTP_HOST or $SCRIPT_NAME is unset");
		return FALSE;
	}
...
static void whitelist_ui_main_html(const char *session)
{
	...
	if (NULL == pfile) {
		system_log_info("[whitelist_ui]: list_file_init %s: %s",
...
static void	whitelist_ui_modify_list(const char *domain, const char *memo)
{
	...
	if (i < item_num) {
		...
		if (-1 == fd) {
			system_log_info("[whitelist_ui]: failed to create %s: %s",
...
static void whitelist_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/domain_whitelist.txt",
