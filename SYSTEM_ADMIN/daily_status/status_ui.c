...
int status_ui_run()
{
	...
	if (g_lang_resource == nullptr) {
		system_log_info("[status_ui]: failed to init language resource");
		return -1;
	}
	...
	if (NULL == request) {
		system_log_info("[status_ui]: $REQUEST_METHOD is unset");
		return -2;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[status_ui]: $REMOTE_ADDR is unset");
		return -3;
	}
	if (0 == strcmp(request, "GET")) {
		query = getenv("QUERY_STRING");
		if (NULL == query) {
			system_log_info("[status_ui]: $QUERY_STRING is unset");
...
static BOOL status_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[status_ui]: $HTTP_HOST or $SCRIPT_NAME is unset");
		return FALSE;
	}
...
static void status_ui_main_html(const char *session, const char *console)
{
	...
	if (NULL == pfile) {
		system_log_info("[status_ui]: list_file_init %s: %s",
