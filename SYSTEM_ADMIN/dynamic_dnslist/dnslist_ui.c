...
typedef struct _DNSLIST_ITEM {
	char domain[256];
	time_t time;
	char memo[256];
} DNSLIST_ITEM;

static void dnslist_ui_encode_line(const char *in, char *out);
	
static void	dnslist_ui_modify_list(const char *domain, const char *memo);

static void dnslist_ui_remove_item(const char *domain);
			
static void dnslist_ui_error_html(const char *error_string);

static void dnslist_ui_main_html(const char *session);
static void dnslist_ui_broadcast_list(void);
static BOOL dnslist_ui_get_self(char *url_buff, int length);

static void dnslist_ui_unencode(char *src, char *last, char *dest);
...
int dnslist_ui_run()
{
	...
	if (g_lang_resource == nullptr) {
		system_log_info("[dnslist_ui]: failed to init language resource");
		return -1;
	}
	language = getenv("HTTP_ACCEPT_LANGUAGE");
	if (NULL == language) {
		dnslist_ui_error_html(NULL);
		return 0;
	}
	request = getenv("REQUEST_METHOD");
	if (NULL == request) {
		system_log_info("[dnslist_ui]: fail to get REQUEST_METHOD"
			" environment!");
		return -2;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[dnslist_ui]: fail to get REMOTE_ADDR environment!");
		return -3;
	}
	if (0 == strcmp(request, "GET")) {
		query = getenv("QUERY_STRING");
		if (NULL == query) {
			system_log_info("[dnslist_ui]: fail to get QUERY_STRING "
				"environment!");
			dnslist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",
				language));
			return 0;
		} else {
			len = strlen(query);
			if (0 == len || len > 1024) {
				system_log_info("[dnslist_ui]: query string too long!");
				dnslist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",
					language));
				return 0;
			}
			dnslist_ui_unencode(query, query + len, search_buff);
			len = strlen(search_buff);
			ptr1 = search_string(search_buff, "session=", len);
			if (NULL == ptr1) {
				system_log_info("[dnslist_ui]: query string of GET "
					"format error");
				dnslist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",
					language));
				return 0;
			...
			if (NULL == ptr2) {
				...
				case ACL_SESSION_TIMEOUT:
					dnslist_ui_error_html(lang_resource_get(g_lang_resource,
			...
			if (NULL == ptr2) {
				...
				dnslist_ui_remove_item(temp_domain);
				dnslist_ui_main_html(session);
				return 0;
			}
			...
			dnslist_ui_modify_list(temp_domain, memo);
			dnslist_ui_main_html(session);
			return 0;
		}
	} else {
		system_log_info("[dnslist_ui]: unrecognized REQUEST_METHOD \"%s\"!",
			request);
		dnslist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST", language));
		return 0;
	}
}
...
static BOOL dnslist_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[dnslist_ui]: fail to get "
...
static void dnslist_ui_main_html(const char *session)
{
	...
	pitem = (DNSLIST_ITEM*)list_file_get_list(pfile);
	...
	for (i=0; i<item_num; i++) {
		...
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].domain,
...
static void	dnslist_ui_modify_list(const char *domain, const char *memo)
{
	...
	if (i < item_num) {
		...
		if (-1 == fd) {
			system_log_info("[dnslist_ui]: fail to create %s", temp_path);
			list_file_free(pfile);
			return;
		}
		...
		if (-1 == fd) {
			system_log_info("[dnslist_ui]: fail to open %s in append mode",
				g_list_path);
			return;
		}
		dnslist_ui_encode_line(memo, temp_memo);
...
static void dnslist_ui_broadcast_list()
{
	DIR *dirp;
	int item_num;
	int i, fd, len;
	...
	if (NULL == dirp){
		system_log_info("[dnslist_ui]: failed to open directory %s: %s",
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/ddns_filter.txt",
			g_mount_path, direntp->d_name);
		fd = open(temp_path, O_CREAT|O_TRUNC|O_WRONLY, DEF_MODE);
		if (-1 == fd) {
			system_log_info("[dnslist_ui]: fail to truncate %s", temp_path);
			continue;
		}
		for (i=0; i<item_num; i++) {
