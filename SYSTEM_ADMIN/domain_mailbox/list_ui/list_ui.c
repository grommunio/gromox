...
function DeleteItem(dmn_name) {location.href='%s?session=%s&dmn_name=' + dmn_name;}\n\
function ModifyItem(dmn_name, destinate, memo) {\
opeform.dmn_name.value=dmn_name; opeform.destinate.value=destinate;\n\
...
name=dmn_name /></TD></TR><TR><TD></TD><TD>%s:</TD><TD><INPUT type=text \n\
value=\"\" tabindex=2 name=destinate /></TD></TR>\n\
<TR><TD></TD><TD>%s:</TD><TD><INPUT type=text value=\"\" tabindex=3 \n\
name=memo /></TD></TR><TR><TD></TD><TD></TD><TD>\n\
<INPUT type=submit tabindex=4 value=\"%s\" onclick=\
\"with (opeform.destinate) {\n\
    apos=value.indexOf('@')\n\
...
&nbsp;%s&nbsp;</TD><TD>&nbsp;<A href=\"javascript:DeleteItem('%s')\">%s</A> | \
...
static void	list_ui_modify_list(const char *dmn_name, const char *destinate,
	const char *memo);

static void list_ui_remove_item(const char *dmn_name);
...
int list_ui_run()
{
	...
	char memo[256];
	char session[256];
	char temp_dmn_name[256];
	char temp_destinate[256];
	...
	if (0 == strcmp(request, "GET")) {
		...
		if (NULL == query) {
			...
			ptr1 += 8;
			ptr2 = search_string(search_buff, "&dmn_name=", len);
			if (NULL == ptr2) {
			...
			ptr1 = ptr2 + 10;
			ptr2 = search_string(search_buff, "&destinate=", len);
			if (NULL == ptr2) {
				if (search_buff + len - ptr1 - 1 > 256 ||
				...
				memcpy(temp_dmn_name, ptr1, search_buff + len - ptr1 - 1);
				temp_dmn_name[search_buff + len - ptr1 - 1] = '\0';
				ltrim_string(temp_dmn_name);
				rtrim_string(temp_dmn_name);
				...
				list_ui_remove_item(temp_dmn_name);
				list_ui_main_html(session);
				return 0;
			}
			...
			memcpy(temp_dmn_name, ptr1, ptr2 - ptr1);
			temp_dmn_name[ptr2 - ptr1] = '\0';
			ltrim_string(temp_dmn_name);
			rtrim_string(temp_dmn_name);
			ptr1 = ptr2 + 11;
			...
			memcpy(temp_destinate, ptr1, ptr2 - ptr1);
			temp_destinate[ptr2 - ptr1] = '\0';
			ltrim_string(temp_destinate);
			rtrim_string(temp_destinate);
			ptr1 = ptr2 + 6;
			if (search_buff + len - ptr1 - 1 > 256) {
				system_log_info("[list_ui]: query string of GET "
					"format error");
			...
			list_ui_modify_list(temp_dmn_name, temp_destinate, memo);
			list_ui_main_html(session);
			return 0;
		}
	} else {
...
static void list_ui_main_html(const char *session)
{
	...
	printf(HTML_MAIN_6, session,
		...
		lang_resource_get(g_lang_resource,"MAIN_RPADDRESS", language),
		lang_resource_get(g_lang_resource,"MAIN_MEMO", language),
		lang_resource_get(g_lang_resource,"ADD_LABEL", language),
		lang_resource_get(g_lang_resource,"MSGERR_RPADDRESS", language));
	...
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, pitem[i].dmn_name, pitem[i].destinate,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language),
				pitem[i].dmn_name, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].dmn_name, pitem[i].destinate, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		} else {
			printf(HTML_TBITEM_NORMAL, pitem[i].dmn_name, pitem[i].destinate,
				pitem[i].memo, pitem[i].dmn_name,
	...
	const char *memo)
{
	...
	item_num = list_file_get_item_num(pfile);
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(dmn_name, pitem[i].dmn_name)) {
			break;
		}
	}
...
static void list_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/delivery/domain_mailbox.txt",
