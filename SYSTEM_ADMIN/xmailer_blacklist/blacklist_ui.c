...
function DeleteItem(xmailer) {location.href='%s?session=%s&xmailer=' + xmailer;}\n\
function ModifyItem(xmailer, type, memo) {\
opeform.xmailer.value=xmailer; opeform.type.value=type; \
opeform.memo.value=memo; opeform.memo.focus();}\n\
...
#define HTML_LOGIN_9	"</CENTER></BODY></HTML>"
...
static void	blacklist_ui_modify_list(const char *xmailer, int type,const char *memo);

static void blacklist_ui_remove_item(const char *xmailer);
...
int blacklist_ui_run()
{
	...
	if (g_lang_resource == nullptr) {
		system_log_info("[blacklist_ui]: failed to init language resource");
		return -1;
	}
	...
	if (NULL == request) {
		system_log_info("[blacklist_ui]: $REQUEST_METHOD is unset");
		return -2;
	}
	remote_ip = getenv("REMOTE_ADDR");
	if (NULL == remote_ip) {
		system_log_info("[blacklist_ui]: $REMOTE_ADDR is unset");
		return -3;
	}
	if (0 == strcmp(request, "GET")) {
		query = getenv("QUERY_STRING");
		if (NULL == query) {
			system_log_info("[blacklist_ui]: $QUERY_STRING is unset");
			...
			ptr1 += 8;
			ptr2 = search_string(search_buff, "&xmailer=", len);
			if (NULL == ptr2) {
			...
			if (NULL == ptr2) {
				...
				memcpy(temp_xmailer, ptr1, search_buff + len - ptr1 - 1);
				temp_xmailer[search_buff + len - ptr1 - 1] = '\0';
				...
				blacklist_ui_remove_item(temp_xmailer);
				blacklist_ui_main_html(session);
				return 0;
			}
			...
			memcpy(temp_xmailer, ptr1, ptr2 - ptr1);
			temp_xmailer[ptr2 - ptr1] = '\0';
			ptr1 = ptr2 + 6;
			ptr2 = search_string(search_buff, "&memo=", len);
			if (NULL == ptr2) {
				system_log_info("[blacklist_ui]: GET query string error, cannot"
					"find memo string ");
				blacklist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",
					language));
				return 0;
			...
			if ('0' == *ptr1) {
				...
				system_log_info("[blacklist_ui]: type in GET query "
					"string error");
				blacklist_ui_error_html(lang_resource_get(g_lang_resource,"ERROR_REQUEST",
					language));
				return 0;
			...
			blacklist_ui_modify_list(temp_xmailer, type, memo);
			blacklist_ui_main_html(session);
			return 0;
		}
	} else {
...
static BOOL blacklist_ui_get_self(char *url_buff, int length)
{
	...
	if (NULL == host || NULL == script) {
		system_log_info("[blacklist_ui]: $HTTP_HOST or $SCRIPT_NAME is unset");
		return FALSE;
	}
...
static void blacklist_ui_main_html(const char *session)
{
	...
	for (i=0; i<item_num; i++) {
		...
		if (0 == strcmp(pitem[i].memo, "none")) {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].xmailer, item_type,
				lang_resource_get(g_lang_resource,"MAIN_NONE", language),
				pitem[i].xmailer, lang_resource_get(g_lang_resource,"DELETE_LABEL", language),
				pitem[i].xmailer, type, pitem[i].memo,
				lang_resource_get(g_lang_resource,"MODIFY_LABEL", language));
		} else {
			printf(HTML_TBITEM_NORMAL, temp_buff, pitem[i].xmailer, item_type,
				pitem[i].memo, pitem[i].xmailer, 
...
static void	blacklist_ui_modify_list(const char *xmailer, int type,
	const char *memo)
{
	int len, fd;
	int i, j, item_num;
	...
	item_num = list_file_get_item_num(pfile);
	for (i=0; i<item_num; i++) {
		if (0 == strcmp(xmailer, pitem[i].xmailer)) {
			break;
		}
	}
	time(&cur_time);
	...
	if (i < item_num) {
		...
		for (j=0; j<item_num; j++) {
			...
			blacklist_ui_encode_line(pitem[j].xmailer, temp_xmailer);
		...
		blacklist_ui_encode_line(xmailer, temp_xmailer);
		blacklist_ui_encode_line(memo, temp_memo);
		if (TYPE_REJECT == type) {
			len = sprintf(temp_line, "%s\t%ld\tREJECT\t%s\n", temp_xmailer,
					cur_time, temp_memo);
		} else {
			len = sprintf(temp_line, "%s\t%ld\tRETRY\t%s\n", temp_xmailer,
					cur_time, temp_memo);
		}
		write(fd, temp_line, len);
		close(fd);
...
static void blacklist_ui_broadcast_list()
{
	...
	while ((direntp = readdir(dirp)) != NULL) {
		...
		sprintf(temp_path, "%s/%s/data/smtp/xmailer_filter.txt",
		...
		for (i=0; i<item_num; i++) {
			blacklist_ui_encode_line(pitem[i].xmailer, temp_xmailer);
			if (0 == strcmp(pitem[i].type, "REJECT")) {
				len = sprintf(temp_line, "M_REJECT\t%s\n", temp_xmailer);
			} else {
				len = sprintf(temp_line, "M_RETRY\t%s\n", temp_xmailer);
			}
			write(fd, temp_line, len);
		}
		close(fd);
